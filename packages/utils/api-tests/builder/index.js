...
const createTestBuilder = (options = {}) => {
  ...
  return {
    ...
    async sanitizedFixtures(balerion) {
      const { fixtures } = this;

      for (const key of Object.keys(fixtures)) {
        fixtures[key] = await this.sanitizedFixturesFor(key, balerion);
      }

      return fixtures;
    },

    sanitizedFixturesFor(modelName, balerion) {
      const model = balerion.getModel(modelsUtils.toContentTypeUID(modelName));
      const fixtures = this.fixturesFor(modelName);

      return balerion.contentAPI.sanitize.output(fixtures, model);
    },
