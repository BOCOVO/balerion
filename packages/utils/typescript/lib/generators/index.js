...
const generate = async (config = {}) => {
  const { pwd, rootDir = TYPES_ROOT_DIR, balerion, artifacts = {}, logger: loggerConfig } = config;
  const reports = {};
  const logger = createLogger(loggerConfig);
  const psTimer = timer().start();

  const registryPwd = path.join(pwd, rootDir, GENERATED_OUT_DIR);
  const generatorConfig = { balerion, pwd: registryPwd, logger };
