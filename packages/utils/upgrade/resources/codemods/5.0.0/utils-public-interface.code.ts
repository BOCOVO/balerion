...
This codemod transforms @balerion/utils imports to change method calls to match the new public interface.
...
import * as utils from '@balerion/utils';
...
import { nameToSlug } from '@balerion/utils';
...
const { strings } = require('@balerion/utils');
...
const { nameToSlug } = require('@balerion/utils');
...
const transformImports = (root: Collection, j: JSCodeshift) => {
  ...
  root.find(j.ImportNamespaceSpecifier).forEach((specifierPath) => {
    if (specifierPath.parent.value.source.value === '@balerion/utils') {
      for (const primitive of Object.keys(changes)) {
        const functions = Object.keys(changes[primitive]);
  ...
  root
    .find(j.VariableDeclarator, {
      init: {
        ...
        arguments: [{ value: '@balerion/utils' }],
      },
    })
    .forEach((path) => {
