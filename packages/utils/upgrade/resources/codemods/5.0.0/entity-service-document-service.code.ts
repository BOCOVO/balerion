...
balerion.entityService.findOne(uid, entityId, {
...
balerion.entityService.findOne(uid, entityId, objectParam_2);
...
balerion.entityService.findOne(...objectParam_5);
...
balerion.entityService.findOne(uid, ...objectParam_6);
...
balerion.entityService.findOne(uid, ...objectParam_7);
...
balerion.entityService.findOne(uid, ...objectParam_8);
...
balerion.entityService.findOne(uid, ...[entityId, [objectParam_9]]);
...
balerion.entityService.findOne(...[...objectParam_10]);

Case: find, create, update, delete with entityId as first argument

balerion.entityService.findMany(uid, {
  fields: ["id", "name", "description"],
  populate: ["author", "comments"],
  publicationState: "preview",
});

balerion.entityService.create(uid, {
  data: {
    name: "John Doe",
    age: 30,
  },
...
balerion.entityService.update(uid, entityId, {
  data: {
    name: "John Doe",
    age: 30,
  },
...
balerion.entityService.delete(uid, entityId);
balerion.entityService.findOne(uid, entityId);
  ...
  root
    ...
    .replaceWith((path) => {
      ...
      return j.callExpression(
        j.memberExpression(
          j.callExpression(j.memberExpression(j.identifier('balerion'), j.identifier('documents')), [
            docUID,
          ]),
