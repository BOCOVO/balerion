 ...
 * For example, `balerion.config.get('plugin.anyString')` will become `balerion.config.get('plugin::anyString')`
 * Ignores api followed by 'rest' or 'responses' because those are the valid Balerion api config values in v4
 */
    ...
    root
      .find(j.CallExpression, {
        callee: {
          ...
          object: {
            type: 'MemberExpression',
            object: { type: 'Identifier', name: 'balerion' },
