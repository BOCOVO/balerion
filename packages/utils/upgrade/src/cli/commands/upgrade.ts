...
export const register = (program: Command) => {
  ...
  addReleaseUpgradeCommand(
    ...
    'Upgrade to the latest available version of Balerion'
  );

  // upgrade major
  addReleaseUpgradeCommand(
    Version.ReleaseType.Major,
    'Upgrade to the next available major version of Balerion'
  );

  // upgrade minor
  addReleaseUpgradeCommand(
    Version.ReleaseType.Minor,
    'Upgrade to the latest minor and patch version of Balerion for the current major'
  );

  // upgrade patch
  addReleaseUpgradeCommand(
    Version.ReleaseType.Patch,
    'Upgrade to latest patch version of Balerion for the current major and minor'
  );

  // upgrade to <target>
  program
    .command('to <target>', { hidden: true })
    .description('Upgrade to the specified version of Balerion')
    .addOption(projectPathOption)
    .addOption(dryOption)
    .addOption(debugOption)
    .addOption(silentOption)
