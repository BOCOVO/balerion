...
export const upgrade = async (options: UpgradeOptions) => {
  ...
  if (!isApplicationProject(project)) {
    throw new Error(
      `The "${options.target}" upgrade can only be run on a Balerion project; for plugins, please use "codemods".`
    );
  }

  logger.debug(
    `Application: VERSION=${f.version(project.packageJSON.version as Version.LiteralVersion)}; BALERION_VERSION=${f.version(project.balerionVersion)}`
  );

  const npmPackage = npmPackageFactory(upgraderConstants.BALERION_PACKAGE_NAME);
