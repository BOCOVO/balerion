...
export const findRangeFromTarget = (
  ...
  // If the current project is a Balerion application
  // Get the range from the given target
  if (isApplicationProject(project)) {
    return getRangeFromTarget(project.balerionVersion, target);
  }

  // Else, if the project is a Balerion plugin or anything else
