...
const currentBalerionVersion = '1.2.3';
...
const appPackageJSONFile = `{
  "name": "test",
  "version": "1.0.0",
  "dependencies": { "@balerion/balerion": "${currentBalerionVersion}" }
}`;
...
describe('Project', () => {
  ...
  describe('Factory', () => {
    ...
    test('Fails when not a plugin and no @balerion/balerion dependency found', async () => {
      vol.fromNestedJSON(
      ...
      expect(() => projectFactory(defaultCWD)).toThrow(
        'No version of @balerion/balerion was found in test. Are you in a valid Balerion project?'
      );
    });

    test(`Use the @balerion/balerion's package.json version as a fallback fails when no version is installed`, () => {
      vol.fromNestedJSON(
        {
          'package.json': `{ "name": "test", "version": "1.2.3", "dependencies": { "@balerion/balerion": "^4.0.0" } }`,
          src: defaultFiles,
        },
        defaultCWD
      );
      ...
      expect(() => projectFactory(defaultCWD)).toThrow(
        `Cannot resolve module "@balerion/balerion" from paths [${defaultCWD}]`
      );
    });
    ...
    test.todo(`Use the @balerion/balerion's package.json version as a fallback succeed`);
    ...
    test('Succeed for valid AppProject', () => {
      ...
      expect(project.balerionVersion.raw).toBe(currentBalerionVersion);
    });
