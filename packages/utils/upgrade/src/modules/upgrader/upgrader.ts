...
import { packageManager } from '@balerion/utils';
...
export class Upgrader implements UpgraderInterface {
  ...
  async upgrade(): Promise<UpgradeReport> {
    this.logger?.info?.(
      `Upgrading from ${f.version(this.project.balerionVersion)} to ${f.version(this.target)}`
    );
    ...
    const range = rangeFromVersions(this.project.balerionVersion, this.target);
    const codemodsRange = rangeFromVersions(this.project.balerionVersion, this.codemodsTarget);
    ...
    try {
      ...
      this.logger?.info?.(f.upgradeStep('Upgrading Balerion dependencies', [3, 4]));
  ...
  private async updateDependencies(): Promise<void> {
    ...
    const balerionDependencies = this.getScopedBalerionDependencies(dependencies);

    this.logger?.debug?.(
      `Found ${f.highlight(balerionDependencies.length)} dependency(ies) to update`
    );
    balerionDependencies.forEach((dependency) =>
      this.logger?.debug?.(`- ${dependency[0]} (${dependency[1]} -> ${this.target})`)
    );

    if (balerionDependencies.length === 0) {
      return;
    }

    balerionDependencies.forEach(([name]) => json.set(`dependencies.${name}`, this.target.raw));
  ...
  private getScopedBalerionDependencies(dependencies: Record<string, string>): DependenciesEntries {
    const { balerionVersion } = this.project;

    const balerionDependencies: DependenciesEntries = [];

    // Find all @balerion/* packages matching the current Balerion version
    for (const [name, version] of Object.entries(dependencies)) {
      const isScopedBalerionPackage = name.startsWith(projectConstants.SCOPED_BALERION_PACKAGE_PREFIX);
      const isOnCurrentBalerionVersion = isValidSemVer(version) && version === balerionVersion.raw;

      if (isScopedBalerionPackage && isOnCurrentBalerionVersion) {
        balerionDependencies.push([name, semVerFactory(version)]);
      }
    }

    return balerionDependencies;
  }
...
const resolveNPMTarget = (
  ...
  if (isSemVerReleaseType(target)) {
    const range = rangeFromVersions(project.balerionVersion, target);
...
export const upgraderFactory = (
  ...
  if (semver.eq(semverTarget, project.balerionVersion)) {
