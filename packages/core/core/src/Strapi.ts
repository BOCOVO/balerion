...
import { Logger, createLogger } from '@balerion/logger';
import { Database } from '@balerion/database';

import type { Core, Modules, UID, Schema } from '@balerion/types';
...
import createBalerionFs from './services/fs';
...
class Balerion extends Container implements Core.Balerion {
  app: any;
  ...
  constructor(opts: BalerionOptions) {
    super();
  ...
  get ee(): Core.Balerion['ee'] {
    return utils.ee;
  }

  get dirs(): Core.BalerionDirectories {
  ...
  /**
   * @deprecated `balerion.entityService` will be removed in the next major version
   */
  ...
  registerInternalServices() {
    ...
    const logger = createLogger({
      level: 'http', // Balerion defaults to level 'http'
      ...config.get('logger'), // DEPRECATED
      ...config.get('server.logger.config'),
    });

    // Instantiate the Balerion container
    this.add('config', () => config)
      ...
      .add('entityService', () => createEntityService({ balerion: this, db: this.db }))
  ...
  async postListen() {
    ...
    this.log.info('Balerion started successfully');
  ...
  async bootstrap() {
    ...
    await this.hook('balerion::content-types.beforeSync').call({
      oldContentTypes,
    ...
    if (this.EE) {
      await utils.ee.checkLicense({ balerion: this });
    }

    await this.hook('balerion::content-types.afterSync').call({
      oldContentTypes,
      contentTypes: this.contentTypes,
    });
  ...
  async destroy() {
    this.log.info('Shutting down Balerion');
    ...
    // @ts-expect-error: Allow clean delete of global.balerion to allow re-instanciation
    delete global.balerion;

    this.log.info('Balerion has been shut down');
  }
  ...
  async runUserLifecycles(lifecycleName: 'register' | 'bootstrap' | 'destroy') {
    ...
    if (isFunction(userLifecycleFunction)) {
      await userLifecycleFunction({ balerion: this });
    }
  }
  ...
  /**
   * @deprecated Use `balerion.db.query` instead
   */
...
export interface BalerionOptions {
  appDir: string;
  distDir: string;
  autoReload?: boolean;
