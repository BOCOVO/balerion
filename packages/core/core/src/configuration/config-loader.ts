...
// the following are restricted to prevent conflicts with existing BALERION_* env vars or root level config options
...
// Existing Balerion configuration files
const BALERION_CONFIG_FILENAMES = [
  'admin',
  'server',
  'api',
  'database',
...
// Note: we don't have access to balerion logger at this point so we can't use it
...
export default (dir: string) => {
  ...
  const configFiles = allFiles.reduce((acc, file) => {
    ...
    // restricted names and Balerion configs are also restricted from being prefixes
    const restrictedPrefix = [...RESTRICTED_FILENAMES, ...BALERION_CONFIG_FILENAMES].find(
      (restrictedName) =>
    ...
    //   logWarning("Using a non-alphanumeric config file name prevents Balerion from auto-loading it from environment variables.")
    // }
