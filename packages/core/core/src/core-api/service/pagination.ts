...
import { errors, pagination } from '@balerion/utils';
...
const getLimitConfigDefaults = () => ({
  defaultLimit: toNumber(balerion.config.get('api.rest.defaultLimit', 25)),
  maxLimit: toNumber(balerion.config.get('api.rest.maxLimit')) || null,
});
...
const shouldCount = (params: { pagination?: PaginationParams }) => {
  ...
  return Boolean(balerion.config.get('api.rest.withCount', true));
};
