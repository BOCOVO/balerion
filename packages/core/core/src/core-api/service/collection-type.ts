import type { Core, Struct, Modules } from '@balerion/types';
  ...
  implements Core.CoreAPI.Service.CollectionType
{
  ...
  async find(params = {}) {
    ...
    const results = await balerion.documents(uid).findMany({
      ...fetchParams,
      ...paginationInfo,
    });

    if (shouldCount(fetchParams)) {
      const count = await balerion.documents(uid).count({ ...fetchParams, ...paginationInfo });
