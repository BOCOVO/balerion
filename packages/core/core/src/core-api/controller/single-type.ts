...
import type { Struct, Core, Utils, UID } from '@balerion/types';
...
const createSingleTypeController = ({
  ...
  return {
    ...
    async find(ctx) {
      ...
      const entity = await balerion.service(uid).find(sanitizedQuery);
    ...
    async update(ctx) {
      ...
      const entity = await balerion.service(uid).createOrUpdate({
        ...query,
    ...
    async delete(ctx) {
      ...
      await balerion.service(uid).delete(query);
