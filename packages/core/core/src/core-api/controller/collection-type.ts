...
import type { Core, Struct, Utils, UID } from '@balerion/types';
...
const createCollectionTypeController = ({
  ...
  return {
    ...
    async find(ctx) {
      ...
      const { results, pagination } = await balerion.service(uid).find(sanitizedQuery);
    ...
    async findOne(ctx) {
      ...
      const entity = await balerion.service(uid).findOne(id, sanitizedQuery);
    ...
    async create(ctx) {
      ...
      const entity = await balerion.service(uid).create({
        ...sanitizedQuery,
    ...
    async update(ctx: Koa.Context) {
      ...
      const entity = await balerion.service(uid).update(id, {
        ...sanitizedQuery,
        data: sanitizedInputData,
      });
    ...
    async delete(ctx) {
      ...
      await balerion.service(uid).delete(id, sanitizedQuery);
