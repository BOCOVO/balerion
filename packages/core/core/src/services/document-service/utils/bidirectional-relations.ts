...
import type { UID, Schema } from '@balerion/types';
 ...
 *
 * In Balerion, bidirectional relations allow maintaining order from both sides of the relation.
...
const load = async (uid: UID.ContentType, { oldVersions }: LoadContext) => {
  ...
  await balerion.db.transaction(async ({ trx }) => {
    const contentTypes = Object.values(balerion.contentTypes) as Schema.ContentType[];
    const components = Object.values(balerion.components) as Schema.Component[];

    for (const model of [...contentTypes, ...components]) {
      const dbModel = balerion.db.metadata.get(model.uid);
      ...
      for (const attribute of Object.values(dbModel.attributes) as any) {
        ...
        const existingRelations = await balerion.db
          .getConnection()
          .select('*')
          .from(joinTable.name)
