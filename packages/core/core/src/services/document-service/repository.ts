...
import { async, contentTypes as contentTypesUtils, validate, errors } from '@balerion/utils';
...
const getModel = ((schema: UID.Schema) => balerion.getModel(schema)) as (schema: string) => any;
...
export const createContentTypeRepository: RepositoryFactoryMethod = (
  ...
  const eventManager = createEventManager(balerion, uid);
  ...
  async function findMany(params = {} as any) {
    ...
    return balerion.db.query(uid).findMany(query);
  }
  ...
  async function findFirst(params = {} as any) {
    ...
    return balerion.db.query(uid).findOne(query);
  }
  ...
  async function deleteDocument(opts = {} as any) {
    ...
    const entriesToDelete = await balerion.db.query(uid).findMany(query);
  ...
  async function clone(opts = {} as any) {
    ...
    // Get deep populate
    const entriesToClone = await balerion.db.query(uid).findMany({
      where: {
        ...queryParams?.lookup,
        documentId,
  ...
  async function update(opts = {} as any) {
    ...
    const entryToUpdate = await balerion.db
      .query(uid)
    ...
    if (!updatedDraft) {
      const documentExists = await balerion.db
  ...
  async function count(params = {} as any) {
    ...
    return balerion.db.query(uid).count(query);
  }
  ...
  async function publish(opts = {} as any) {
    ...
    const [draftsToPublish, oldPublishedVersions] = await Promise.all([
      balerion.db.query(uid).findMany({
        where: {
          ...queryParams?.lookup,
          documentId,
  ...
  async function unpublish(opts = {} as any) {
    ...
    const versionsToDelete = await balerion.db.query(uid).findMany(query);
