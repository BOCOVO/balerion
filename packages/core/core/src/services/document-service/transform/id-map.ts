import type { Core, Data, UID } from '@balerion/types';
 ...
 */
const createIdMap = ({ balerion }: { balerion: Core.Balerion }): IdMap => {
    ...
    async load() {
      ...
      await async.map(
        ...
        async ({ uid, locale, documentIds, status }: any) => {
          ...
          const result = await balerion?.db?.query(uid).findMany(findParams);
