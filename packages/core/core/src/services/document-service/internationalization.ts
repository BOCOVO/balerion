import type { Struct, Modules } from '@balerion/types';
...
const defaultLocale: AsyncTransform = async (contentType, params) => {
  if (!balerion.plugin('i18n').service('content-types').isLocalizedContentType(contentType)) {
    return params;
  }
...
const localeToLookup: Transform = (contentType, params) => {
  if (
    !params.locale ||
    !balerion.plugin('i18n').service('content-types').isLocalizedContentType(contentType)
  ) {
    return params;
  }
