...
type TaskFn = ({ balerion }: { balerion: Core.Balerion }, ...args: unknown[]) => Promise<unknown>;
  ...
  return {
    add(tasks: Tasks = {}) {
      for (const taskExpression of Object.keys(tasks)) {
        ...
        const fnWithBalerion = (...args: unknown[]) => fn({ balerion }, ...args);

        // const job = new Job(null, fnWithBalerion);
        const job = new Job(fnWithBalerion);
