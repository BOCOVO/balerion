...
import balerionUtils from '@balerion/utils';
import type { Modules, UID, Struct, Schema } from '@balerion/types';
import { Validators, ValidatorMetas } from './validators';

type CreateOrUpdate = 'creation' | 'update';

const { yup, validateYupSchema } = balerionUtils;
const { isMediaAttribute, isScalarAttribute, getWritableAttributes } = balerionUtils.contentTypes;
const { ValidationError } = balerionUtils.errors;
...
const addRequiredValidation = (createOrUpdate: CreateOrUpdate) => {
  return <T extends balerionUtils.yup.AnySchema>(
    validator: T,
    {
      attr: { required },
...
const addDefault = (createOrUpdate: CreateOrUpdate) => {
  return (
    validator: balerionUtils.yup.BaseSchema,
...
const preventCast = (validator: balerionUtils.yup.AnySchema) =>
...
const createComponentValidator =
  ...
  (
    ...
    const model = balerion.getModel(attr.component);
    if (!model) {
...
const createDzValidator =
  ...
  ({ attr, updatedAttribute, componentContext }: ValidatorMeta, { isDraft }: ValidatorContext) => {
    ...
    validator = yup.array().of(
      yup.lazy((item) => {
        const model = balerion.getModel(prop('__component', item));
        const schema = yup
          .object()
          .shape({
            __component: yup.string().required().oneOf(Object.keys(balerion.components)),
          })
          .notNull();
...
const createModelValidator =
  ...
  ({ componentContext, model, data, entity }: ModelValidatorMetas, options: ValidatorContext) => {
    ...
    const schema = writableAttributes.reduce(
      ...
      {} as Record<string, balerionUtils.yup.BaseSchema>
    );
...
const buildRelationsStore = <TUID extends UID.Schema>({
  ...
  const currentModel = balerion.getModel(uid);
...
const checkRelationsExist = async (relationsStore: Record<string, ID[]> = {}) => {
  ...
  for (const [key, value] of Object.entries(relationsStore)) {
    const evaluate = async () => {
      ...
      const count = await balerion.db.query(key as UID.Schema).count({
        where: {
          id: {
