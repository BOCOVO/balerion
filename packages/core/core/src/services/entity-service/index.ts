...
import { errors as databaseErrors } from '@balerion/database';
import {
...
import type { Core, Modules, Utils } from '@balerion/types';
...
const createDefaultImplementation = ({
  ...
  async findMany(uid, opts) {
    const { kind } = balerion.getModel(uid);

    const wrappedParams = await this.wrapParams(opts, { uid, action: 'findMany' });

    if (kind === 'singleType') {
      const entity = balerion.documents!(uid).findFirst(wrappedParams);
      return this.wrapResult(entity, { uid, action: 'findOne' });
    }

    const entities = await balerion.documents!(uid).findMany(wrappedParams);
  ...
  async findPage(uid, opts) {
    ...
    const query = balerion.get('query-params').transform(uid, wrappedParams);
  ...
  async findOne(uid, entityId, opts) {
    ...
    const entity = await balerion.documents!(uid).findOne({
      ...wrappedParams,
  ...
  async count(uid, opts) {
    ...
    return balerion.documents!(uid).count(wrappedParams);
  },
  ...
  async create(uid, params) {
    ...
    const shouldPublish = !contentTypesUtils.isDraft(data, balerion.getModel(uid));

    const entity = await balerion.documents!(uid).create({
  ...
  async update(uid, entityId, opts) {
    ...
    const shouldPublish = !contentTypesUtils.isDraft(entityToUpdate, balerion.getModel(uid));

    const entity = balerion.documents!(uid).update({
      ...(wrappedParams as any),
      status: shouldPublish ? 'published' : 'draft',
