...
import type { Database, Model } from '@balerion/database';

const coreStoreModel: Model = {
  uid: 'balerion::core-store',
  singularName: 'balerion_core_store_settings',
...
const createCoreStore = ({ db }: { db: Database }) => {
  ...
  store.get = async (params) => {
    ...
    const data = await db.query('balerion::core-store').findOne({ where });
  ...
  store.set = async (params) => {
    ...
    if (data) {
      return db.query('balerion::core-store').update({
        where: { id: data.id },
        data: {
    ...
    return db.query('balerion::core-store').create({
      data: {
        ...where,
        value: JSON.stringify(value) || toString(value),
        type: typeof value,
  ...
  store.delete = async (params) => {
    ...
    return db.query('balerion::core-store').delete({ where });
  };
