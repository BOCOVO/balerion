...
import type { Model, Database } from '@balerion/database';
import type { Modules } from '@balerion/types';

const { ValidationError } = errors;

const webhookModel: Model = {
  uid: 'balerion::webhook',
  singularName: 'balerion_webhooks',
  tableName: 'balerion_webhooks',
  attributes: {
    id: {
      type: 'increments',
    },
...
const createWebhookStore = ({ db }: { db: Database }): WebhookStore => {
  return {
    ...
    async findWebhooks() {
      const results = await db.query('balerion::webhook').findMany();

      return results.map(fromDBObject);
    },
    async findWebhook(id) {
      const result = await db.query('balerion::webhook').findOne({ where: { id } });
    ...
    async updateWebhook(id, data) {
      ...
      const webhook = await db.query('balerion::webhook').update({
        where: { id },
    ...
    async deleteWebhook(id) {
      const webhook = await db.query('balerion::webhook').delete({ where: { id } });
      return webhook ? fromDBObject(webhook) : null;
    },
  };
};
