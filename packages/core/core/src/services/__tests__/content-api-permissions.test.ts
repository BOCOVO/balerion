...
describe('Content API - Permissions', () => {
  ...
  describe('Get Actions Map', () => {
    test('When no API are defined, it should return an empty object', () => {
      global.balerion = balerionMock as any;

      const contentAPI = createContentAPI(global.balerion);
  ...
  describe('Providers', () => {
    test('You should not be able to register action once balerion is loaded', () => {
      global.balerion.isLoaded = true;
  ...
  describe('Engine', () => {
    test('Engine warns when registering an unknown action', async () => {
      ...
      expect(global.balerion.log.debug).toHaveBeenCalledWith(
    ...
    test('Engine filter out invalid action when generating an ability', async () => {
      ...
      expect(global.balerion.log.debug).toHaveBeenCalledTimes(1);
      expect(global.balerion.log.debug).toHaveBeenCalledWith(
        `Unknown action "foo" supplied when registering a new permission`
      );
    });
