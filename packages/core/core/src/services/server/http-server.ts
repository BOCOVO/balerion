...
const createHTTPServer = (balerion: Core.Balerion, koaApp: Koa): Server => {
  ...
  const options = balerion.config.get<http.ServerOptions>('server.http.serverOptions', {});
  ...
  server.on('error', (err) => {
    if ('code' in err && 'port' in err && err.code === 'EADDRINUSE') {
      return balerion.stopWithError(`The port ${err.port} is already used by another application.`);
    }
