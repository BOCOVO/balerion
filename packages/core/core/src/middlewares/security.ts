...
const defaults: Config = {
  ...
  contentSecurityPolicy: {
    ...
    directives: {
      ...
      'img-src': ["'self'", 'data:', 'blob:', 'https://market-assets.balerion.io'],
...
export const security: Core.MiddlewareFactory<Config> =
  ...
  (ctx, next) => {
    ...
    const directives: {
      ...
      'img-src': ["'self'", 'data:', 'cdn.jsdelivr.net', 'balerion.io'],
      'manifest-src': [],
      'frame-src': [],
    };

    // if apollo graphql playground is enabled, add exceptions for it
    if (balerion.plugin('graphql')?.service('utils').playground.isEnabled()) {
      const { config: gqlConfig } = balerion.plugin('graphql');
    ...
    if (
      ...
      ctx.method === 'GET' &&
      ctx.path.startsWith(balerion.config.get('admin.path'))
    ) {
      helmetConfig = mergeConfig(helmetConfig, {
        contentSecurityPolicy: {
          directives: {
