...
export const cors: Core.MiddlewareFactory<Config> = (config) => {
  ...
  if (config.enabled !== undefined) {
    balerion.log.warn(
      'The balerion::cors middleware no longer supports the `enabled` option. Using it' +
        ' to conditionally enable CORS might cause an insecure default. To disable balerion::cors, remove it from' +
