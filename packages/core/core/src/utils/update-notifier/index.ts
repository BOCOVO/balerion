...
const getUpdateMessage = (newVersion: string, currentVersion: string) => {
  ...
  const releaseLink = chalk.bold('https://github.com/balerion/balerion/releases');

  return `
A new version of Balerion is available ${currentVersionLog} → ${newVersionLog}
Check out the new releases at: ${releaseLink}
`.trim();
};

export const createUpdateNotifier = (balerion: Core.Balerion) => {
  ...
  try {
    config = new Configstore(
      pkg.name,
      {},
      { configPath: path.join(balerion.dirs.app.root, '.balerion-updater.json') }
    );
  } catch {
  ...
  if (env.bool('BALERION_DISABLE_UPDATE_NOTIFICATION', false)) {
    balerion.log.warn(
      'BALERION_DISABLE_UPDATE_NOTIFICATION is no longer supported. Instead, set logger.updates.enabled to false in your server configuration.'
    );
  }

  if (!balerion.config.get('server.logger.updates.enabled') || !config) {
    return;
  }
