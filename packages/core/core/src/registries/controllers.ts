...
export type ControllerFactory =
  | ((params: { balerion: Core.Balerion }) => Core.Controller)
  | Core.Controller;
...
const controllersRegistry = (balerion: Core.Balerion) => {
  ...
  return {
    ...
    get(uid: UID.Controller) {
      ...
      if (controller) {
        instances[uid] = typeof controller === 'function' ? controller({ balerion }) : controller;
        return instances[uid];
      }
    },
