...
const customFieldsRegistry = (balerion: Core.Balerion) => {
    ...
    add(
      ...
      for (const cf of customFieldList) {
        ...
        if (!ALLOWED_TYPES.includes(type)) {
          throw new Error(
            `Custom field type: '${type}' is not a valid Balerion type or it can't be used with a Custom Field`
          );
        }
        ...
        // When no plugin is specified, or it isn't found in Balerion, default to global
        const uid =
          plugin && balerion.plugin(plugin) ? `plugin::${plugin}.${name}` : `global::${name}`;
