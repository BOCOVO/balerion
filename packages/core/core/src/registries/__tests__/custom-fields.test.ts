...
const balerion = {
  ...
  plugin: jest.fn((plugin) => balerion.plugins[plugin]),
};
...
describe('Custom fields registry', () => {
  describe('add', () => {
    it('adds a custom field registered in a plugin', () => {
      ...
      const customFields = customFieldsRegistry(balerion);
    ...
    it('validates the type is a Balerion type', () => {
      const mockCF = {
        name: 'test',
        type: 'geojson',
      };
      ...
      expect(() => customFields.add(mockCF)).toThrowError(
        `Custom field type: 'geojson' is not a valid Balerion type`
      );
    });
