...
export type ServiceFactory = (params: { balerion: Core.Balerion }) => Core.Service | Core.Service;
...
const servicesRegistry = (balerion: Core.Balerion) => {
  ...
  return {
    ...
    get(uid: UID.Service) {
      ...
      if (service) {
        instantiatedServices[uid] = typeof service === 'function' ? service({ balerion }) : service;
        return instantiatedServices[uid];
      }
    },
