...
import { strings, importDefault } from '@balerion/utils';
...
export default async function loadAPIs(balerion: Core.Balerion) {
  if (!existsSync(balerion.dirs.dist.api)) {
    return;
  }

  const apisFDs = await (await fse.readdir(balerion.dirs.dist.api, { withFileTypes: true }))
    .filter(isDirectory)
  ...
  for (const apiFD of apisFDs) {
    ...
    const api = await loadAPI(apiName, join(balerion.dirs.dist.api, apiFD.name));
  ...
  for (const apiName of Object.keys(apis)) {
    balerion.get('apis').add(apiName, apis[apiName]);
  }
}
