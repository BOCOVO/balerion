...
const watch = async (ctx: BuildContext): Promise<ViteWatcher> => {
  ...
  if (typeof hmrConfig === 'object' && hmrConfig.server === hmrServer) {
    // Only restart the hmr server when Balerion's server is listening
    balerion.server.httpServer.on('listening', async () => {
  ...
  const serveAdmin: Core.MiddlewareHandler = async (koaCtx, next) => {
    ...
    let template = await fs.readFile(path.relative(ctx.cwd, '.balerion/client/index.html'), 'utf-8');
  ...
  ctx.balerion.server.router.get(adminRoute, serveAdmin);
  ctx.balerion.server.router.use(adminRoute, viteMiddlewares);
