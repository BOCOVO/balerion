...
import { loadBalerionMonorepo } from '../core/monorepo';
...
const resolveBaseConfig = async (ctx: BuildContext): Promise<InlineConfig> => {
  ...
  return {
    ...
    cacheDir: 'node_modules/.balerion/vite',
    configFile: false,
    define: {
      'process.env': ctx.env,
    },
    envPrefix: 'BALERION_ADMIN_',
    optimizeDeps: {
      include: [
...
const resolveDevelopmentConfig = async (ctx: BuildContext): Promise<InlineConfig> => {
  const monorepo = await loadBalerionMonorepo(ctx.cwd);
  const baseConfig = await resolveBaseConfig(ctx);
