import * as tsUtils from '@balerion/typescript-utils';
...
const develop = async ({
  ...
  if (cluster.isWorker) {
    timer.start('loadBalerion');
    const loadBalerionSpinner = logger.spinner(`Loading Balerion`).start();

    const balerion = createBalerion({
      appDir: cwd,
     ...
     * as a balerion middleware.
     */
    let bundleWatcher: WebpackWatcher | ViteWatcher | undefined;

    const balerionInstance = await balerion.load();
    ...
    const loadBalerionDuration = timer.end('loadBalerion');
    loadBalerionSpinner.text = `Loading Balerion (${prettyTime(loadBalerionDuration)})`;
    loadBalerionSpinner.succeed();
    ...
    if (tsconfig?.config || balerion.config.get('typescript.autogenerate') !== false) {
    ...
    const restart = async () => {
      if (balerionInstance.reload.isWatching && !balerionInstance.reload.isReloading) {
        balerionInstance.reload.isReloading = true;
        balerionInstance.reload();
      }
    };
    ...
    const watcher = chokidar
      .watch(cwd, {
        ...
        ignored: [
          ...
          '**/public',
          '**/public/**',
          balerionInstance.dirs.static.public,
          strings.joinBy('/', balerionInstance.dirs.static.public, '**'),
          '**/*.db*',
          '**/exports/**',
          '**/dist/**',
          '**/*.d.ts',
          ...
          ...balerionInstance.config.get('admin.watchIgnoreFiles', []),
        ],
      })
      .on('add', (path) => {
        balerionInstance.log.info(`File created: ${path}`);
        restart();
      })
      .on('change', (path) => {
        balerionInstance.log.info(`File changed: ${path}`);
        restart();
      })
      .on('unlink', (path) => {
        balerionInstance.log.info(`File deleted: ${path}`);
        restart();
      });
    ...
    process.on('message', async (message) => {
      switch (message) {
        case 'kill': {
          logger.debug(
            'child process has the kill message, destroying the balerion instance and sending the killed process message'
          );
          await watcher.close();

          await balerionInstance.destroy();
