...
const {
  providers: { createLocalBalerionDestinationProvider, DEFAULT_CONFLICT_STRATEGY },
 ...
 * Import command.
 *
 * It transfers data from a file to a local Balerion instance
 */
export default async (opts: CmdOptions) => {
  ...
  /**
   * To local Balerion instance
   */
  const balerionInstance = await createBalerionInstance();
  ...
  const destination = createLocalBalerionDestinationProvider(destinationOptions);
  ...
  progress.on('transfer::start', async () => {
    ...
    await balerionInstance.telemetry.send(
  ...
  try {
    ...
    await balerionInstance.telemetry.send('didDEITSProcessFail', getTransferTelemetryPayload(engine));
