...
import { commands as balerionCommands } from './commands';
...
const createCLI = async (argv: string[], command = new Command()) => {
  ...
  // Load all commands
  for (const commandFactory of balerionCommands) {
    try {
  ...
  const deprecatedCommands = [
    ...
    {
      name: 'plugin:verify',
      message: 'After migrating your plugin to v5, use `balerion-plugin verify`',
    },
    {
      name: 'plugin:watch',
      message: 'After migrating your plugin to v5, use `balerion-plugin watch`',
    },
    {
      name: 'plugin:watch:link',
      message: 'After migrating your plugin to v5, use `balerion-plugin watch:link`',
    },
    {
      name: 'plugin:build',
      message: 'After migrating your plugin to v5, use `balerion-plugin build`',
    },
  ];
  ...
  deprecatedCommands.forEach(({ name, message }) => {
    const deprecated = new Command(name)
      ...
      .action(() => {
        console.warn(
          `The command ${name} has been deprecated. See the Balerion 5 migration guide for more information.`
        );
        if (message) {
