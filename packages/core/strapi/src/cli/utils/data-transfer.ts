...
import { configs, createLogger, type winston, formats } from '@balerion/logger';
...
const setSignalHandler = async (
  ...
  signals.forEach((signal) => {
    // We specifically remove ALL listeners because we have to clear the one added in Balerion bootstrap that has a process.exit
    // TODO: Ideally Balerion bootstrap would not add that listener, and then this could be more flexible and add/remove only what it needs to
    process.removeAllListeners(signal);
    process.on(signal, handler);
  });
};

const createBalerionInstance = async (opts: { logLevel?: string } = {}): Promise<Core.Balerion> => {
  try {
    const appContext = await compileBalerion();
    const app = createBalerion({ ...opts, ...appContext });

    app.log.level = opts.logLevel || 'error';
    return await app.load();
  } catch (error) {
    if (error instanceof Error && 'code' in error && error.code === 'ECONNREFUSED') {
      throw new Error('Process failed. Check the database connection with your Balerion project.');
    }
...
const getDiffHandler = (
  ...
  return async (
    ...
    setSignalHandler(async () => {
      await abortTransfer({ engine, balerion: balerion as Core.Balerion });
    ...
    Object.entries(context.diffs).forEach(([uid, diffs]) => {
      for (const diff of diffs) {
        ...
        if (
          ...
          endPath?.startsWith('balerion_stage') ||
          endPath?.startsWith('balerion_assignee')
        ) {
    ...
    setSignalHandler(() => abortTransfer({ engine, balerion: balerion as Core.Balerion }));
...
const getAssetsBackupHandler = (
  ...
  return async (
    ...
    console.warn(
      'The backup for the assets could not be created inside the public directory. Ensure Balerion has write permissions on the public directory.'
    );
    const confirmed = await confirmMessage(
...
type RestoreConfig = NonNullable<
  balerionDataTransfer.providers.ILocalBalerionDestinationProviderOptions['restore']
>;
...
const parseRestoreFromOptions = (opts: Partial<engineDataTransfer.ITransferEngineOptions>) => {
  ...
  const restoreConfig: balerionDataTransfer.providers.ILocalBalerionDestinationProviderOptions['restore'] =
    {
