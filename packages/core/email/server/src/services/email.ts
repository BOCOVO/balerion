...
import { objects, template } from '@balerion/utils';
...
const getProviderSettings = (): EmailConfig => balerion.config.get('plugin::email');

const send = async (options: SendOptions) => balerion.plugin('email').provider.send(options);
...
const sendTemplatedEmail = (
  ...
  return balerion.plugin('email').provider.send({ ...emailOptions, ...templatedAttributes });
};
