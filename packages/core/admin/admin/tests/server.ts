...
export const server: SetupServer = setupServer(
  ...[
     ...
     * TRACKING
     */
    rest.post('https://analytics.balerion.io/api/v2/track', (req, res, ctx) => {
      ...
      res(
        ctx.json({
          data: [
            {
              id: 1,
              code: 'balerion-editor',
              name: 'Editor',
            },
            {
              id: 2,
              code: 'balerion-author',
              name: 'Author',
            },
          ],
        })
      ...
      res(
        ctx.json({
          data: {
            ...
            roles: [
              { id: 1, code: 'balerion-editor', name: 'Editor' },
              { id: 2, code: 'balerion-super-admin', name: 'Super Admin' },
            ],
            params: {
      ...
      return res(
        ctx.json({
          data: {
            ...
            email: 'test+register-token@balerion.io',
          },
        })
      );
    }),
     ...
     *
     */
    rest.get('https://market-api.balerion.io/providers', (req, res, ctx) => {
      const { collections = [], search = '' } = qs.parse(req.url.searchParams.toString()) as {
        collections: Array<keyof MockData['marketplace']['providers']>;
        search: string;
      };

      const [madeByBalerion, verified] = collections;

      let responseData;

      const providerResponses = mockData.marketplace.providers;

      if (madeByBalerion && verified) {
        responseData = {
          data: [...providerResponses[madeByBalerion].data, ...providerResponses[verified].data],
    ...
    rest.get('https://market-api.balerion.io/plugins', (req, res, ctx) => {
      const {
        collections = [],
        categories = [],
        search = '',
      ...
      } else if (madeByBalerion && verified) {
        responseData = {
          data: [...pluginResponses[madeByBalerion].data, ...pluginResponses[verified].data],
          meta: pluginResponses.plugins.meta,
        };
     ...
     *
     */
    rest.post('https://analytics.balerion.io/submit-nps', (req, res, ctx) => {
      return res(ctx.status(200));
    }),
    /**
