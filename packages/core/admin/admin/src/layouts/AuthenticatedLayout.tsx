...
import packageInfo from '@balerion/admin/package.json';
...
const { version: balerionVersion } = packageInfo;
...
const AdminLayout = () => {
  ...
  const [tagName, setTagName] = React.useState<string>(balerionVersion);

  React.useEffect(() => {
    if (showReleaseNotification) {
      fetch('https://api.github.com/repos/balerion/balerion/releases/latest')
        .then(async (res) => {
          if (!res.ok) {
            return;
          }
        ...
        .catch(() => {
          /**
           * silence is golden & we'll use the balerionVersion as a fallback
           */
        });
    }
  ...
  React.useEffect(() => {
    if (userRoles) {
      const isUserSuperAdmin = userRoles.find(({ code }) => code === 'balerion-super-admin');
  ...
  const {
    ...
    generalSectionLinks,
    pluginsSectionLinks,
  } = useMenu(checkLatestBalerionVersion(balerionVersion, tagName));
  ...
  return (
    <AppInfoProvider
      {...appInfo}
      userId={userId}
      latestBalerionReleaseTag={tagName}
      shouldUpdateBalerion={checkLatestBalerionVersion(balerionVersion, tagName)}
    >
      <NpsSurvey />
      <PluginsInitializer>
...
const checkLatestBalerionVersion = (
