...
describe('Webhooks | ListPage', () => {
  ...
  it('should show a loader when data is loading and then display the data', async () => {
    ...
    await waitFor(async () => {
      expect(getByText('http:://balerion.io')).toBeInTheDocument();
    });
  });
  ...
  it('should delete a single webhook', async () => {
    ...
    await findByText('http:://balerion.io');
    ...
    await waitFor(() => expect(queryByText('http:://balerion.io')).not.toBeInTheDocument());
  });
