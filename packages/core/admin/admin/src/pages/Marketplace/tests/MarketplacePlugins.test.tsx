...
describe('Marketplace page - plugins tab', () => {
  ...
  it('filters a collection option', async () => {
    ...
    await user.click(getByRole('button', { name: 'Filters' }));
    await user.click(getByRole('combobox', { name: 'Collections' }));
    await user.click(getByRole('option', { name: 'Made by Balerion (13)' }));
    // Close the combobox
    ...
    expect(getByText('Made by Balerion')).toBeVisible();
  ...
  it('removes a filter option tag', async () => {
    ...
    expect(screen.getByText('?collections[0]=Made by Balerion&page=1')).toBeInTheDocument();
    const removeButton = getByText('Made by Balerion').nextElementSibling;
