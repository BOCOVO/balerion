...
describe('Profile page', () => {
  const originalIsEnabled = window.balerion.features.isEnabled;
  const originalIsEE = window.balerion.isEE;

  beforeAll(() => {
    window.balerion.isEE = true;
    window.balerion.features.isEnabled = () => true;

    window.localStorage.setItem('jwtToken', JSON.stringify('token'));
  });

  afterAll(() => {
    window.balerion.isEE = originalIsEE;
    window.balerion.features.isEnabled = originalIsEnabled;
  });
