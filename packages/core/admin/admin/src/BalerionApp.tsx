...
import Logo from './assets/images/logo-balerion-2022.svg';
...
import { Router, BalerionAppSetting, UnloadedSettingsLink } from './core/apis/router';
...
interface BalerionAppConstructorArgs extends Partial<Pick<BalerionApp, 'appPlugins'>> {
  config?: {
    auth?: { logo: string };
    head?: { favicon: string };
    locales?: string[];
...
interface BalerionAppPlugin {
  bootstrap?: (
    args: Pick<BalerionApp, 'addSettingsLink' | 'addSettingsLinks' | 'getPlugin' | 'registerHook'>
  ) => void;
  register: (app: BalerionApp) => void;
...
class BalerionApp {
  appPlugins: Record<string, BalerionAppPlugin>;
  ...
  translations: BalerionApp['configurations']['translations'] = {};
  ...
  constructor({ config, appPlugins }: BalerionAppConstructorArgs = {}) {
  ...
  addMiddlewares = (middlewares: BalerionApp['middlewares']) => {
  ...
  createSettingSection = (
    section: Pick<BalerionAppSetting, 'id' | 'intlLabel'>,
  ...
  createCustomConfigurations = (
    customConfig: NonNullable<BalerionAppConstructorArgs['config']>
  ) => {
    ...
    if (customConfig.theme) {
      ...
      if (!darkTheme && !lightTheme) {
        console.warn(
          `[deprecated] In future versions, Balerion will stop supporting this theme customization syntax. The theme configuration accepts a light and a dark key to customize each theme separately. See https://docs.balerion.io/developer-docs/latest/development/admin-customization.html#theme-extension.`.trim()
        );
  ...
  render() {
    ...
    this.store = configureStore(
      {
        admin_app: {
          permissions: merge({}, ADMIN_PERMISSIONS_CE),
          theme: {
            availableThemes: [],
...
export { BalerionApp };
export type { BalerionAppPlugin, BalerionAppConstructorArgs, InjectionZoneComponent };
