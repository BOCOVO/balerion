...
const TrackingProvider = ({ children }: TrackingProviderProps) => {
  ...
  React.useEffect(() => {
    if (uuid && data) {
      ...
      try {
        fetch('https://analytics.balerion.io/api/v2/track', {
          method: 'POST',
          body: JSON.stringify({
...
interface EventWithoutProperties {
  name:
    ...
    | 'didClickOnTryBalerionCloudSection'
...
/**
 * @description Used to send amplitude events to the Balerion Tracking hub.
 *
 * @example
 * ```tsx
 * import { useTracking } from '@balerion/balerion/admin';
 *
 * const MyComponent = () => {
...
const useTracking = (): UseTrackingReturn => {
  ...
  const trackUsage = React.useCallback(
    async <TEvent extends TrackingEvent>(
      ...
      try {
        if (uuid && !window.balerion.telemetryDisabled) {
          const res = await axios.post<string>(
            'https://analytics.balerion.io/api/v2/track',
            {
              event,
              userId,
              ...
              groupProperties: {
                ...telemetryProperties,
                projectId: uuid,
                projectType: window.balerion.projectType,
              },
            },
            {
              headers: {
