...
import type { errors } from '@balerion/utils';
 ...
 * to the Balerion backend.
 * @example
 * ```tsx
 * import { getFetchClient } from '@balerion/admin/admin';
 *
 * const myFunct = () => {
...
const getFetchClient = (defaultOptions: FetchConfig = {}): FetchClient => {
  const backendURL = window.balerion.backendURL;
  const defaultHeader = {
  ...
  const fetchClient: FetchClient = {
    get: async <TData>(url: string, options?: FetchOptions): Promise<FetchResponse<TData>> => {
       ...
       * - appending our BaseURL which comes from the window.balerion object
