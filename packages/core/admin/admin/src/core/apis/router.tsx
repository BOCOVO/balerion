...
import { BalerionApp } from '../../BalerionApp';
...
interface BalerionAppSettingLink extends Omit<MenuItem, 'icon' | 'notificationCount'> {
  id: string;
}

interface UnloadedSettingsLink extends Omit<BalerionAppSettingLink, 'Component'> {
  Component?: () => Promise<{ default: React.ComponentType }>;
}

interface BalerionAppSetting {
  id: string;
  intlLabel: MessageDescriptor & {
    values?: Record<string, PrimitiveType>;
  };
  links: Omit<BalerionAppSettingLink, 'Component'>[];
}
...
class Router {
  ...
  private _settings: Record<string, BalerionAppSetting> = {
    global: {
      id: 'global',
      intlLabel: {
        id: 'Settings.global',
  ...
  /**
   * @internal This method is used internally by Balerion to create the router.
   * It should not be used by plugins, doing so will likely break the application.
   */
  createRouter(balerion: BalerionApp, { memory, ...opts }: RouterOptions = {}) {
    const routes = [
      {
        path: '/*',
        errorElement: (
          <Provider store={balerion.store!}>
        ...
        element: <App balerion={balerion} store={balerion.store!} />,
        children: [
          ...getImmutableRoutes(),
          {
            path: '/*',
  ...
  public addMenuLink = (
    ...
    if (link.to.startsWith('/')) {
      console.warn(
        `[${link.intlLabel.defaultMessage}]: the \`to\` property of your menu link is an absolute path, it should be relative to the root of the application. This has been corrected for you but will be removed in a future version of Balerion.`
      );
  ...
  public addSettingsLink(
    section: Pick<BalerionAppSetting, 'id' | 'intlLabel'> & { links: UnloadedSettingsLink[] },
    links?: never
  ): void;
  public addSettingsLink(
    sectionId: string | Pick<BalerionAppSetting, 'id' | 'intlLabel'>,
  ...
  public addSettingsLink(
    section:
      | string
      | Pick<BalerionAppSetting, 'id' | 'intlLabel'>
      | (Pick<BalerionAppSetting, 'id' | 'intlLabel'> & { links: UnloadedSettingsLink[] }),
  ...
  private createSettingsLink = (sectionId: string, link: UnloadedSettingsLink) => {
    ...
    if (link.to.startsWith('/')) {
      console.warn(
        `[${link.intlLabel.defaultMessage}]: the \`to\` property of your settings link is an absolute path. It should be relative to \`/settings\`. This has been corrected for you but will be removed in a future version of Balerion.`
      );
    ...
    if (link.to.split('/')[0] === 'settings') {
      console.warn(
        `[${link.intlLabel.defaultMessage}]: the \`to\` property of your settings link has \`settings\` as the first part of it's path. It should be relative to \`settings\` and therefore, not include it. This has been corrected for you but will be removed in a future version of Balerion.`
      );
...
export { Router };
export type { MenuItem, BalerionAppSettingLink, UnloadedSettingsLink, BalerionAppSetting, RouteObject };
