...
const checkIfShouldShowSurvey = (settings: NpsSurveySettings) => {
  ...
  // 1. If the survey is disabled by balerion, abort mission, don't bother checking the other settings.
  ...
  // Global balerion disable for NSP.
  if (window.balerion.flags.nps === false) {
    return false;
  }
...
const NpsSurvey = () => {
  ...
  const balerionVersion = useAppInfo('NpsSurvey', (state) => state.balerionVersion);
  ...
  const handleSubmitResponse = async ({
    ...
    try {
      const body = {
        ...
        version: balerionVersion ?? undefined,
        license: window.balerion.projectType,
        isHostedOnBalerionCloud: process.env.BALERION_HOSTING === 'balerion.cloud',
      };
      const res = await fetch('https://analytics.balerion.io/submit-nps', {
        method: 'POST',
        headers: {
                    ...
                    <Box marginLeft="auto" marginRight="auto">
                      <Typography fontWeight="semiBold" tag="legend">
                        {formatMessage({
                          ...
                          defaultMessage:
                            'How likely are you to recommend Balerion to a friend or colleague?',
                        })}
                      </Typography>
                    </Box>
                    <IconButton
