...
const hash = (accessKey: string) => {
  return crypto
    .createHmac('sha512', balerion.config.get('admin.apiToken.salt'))
    .update(accessKey)
    .digest('hex');
};
...
const create = async (attributes: ApiTokenBody): Promise<ApiToken> => {
  ...
  // Create the token
  const apiToken: ApiToken = await balerion.db.query('admin::api-token').create({
  ...
  if (attributes.type === constants.API_TOKEN_TYPE.CUSTOM) {
    ...
    // const permissionsCount = await balerion.db.query('admin::api-token-permission').createMany({
    //   populate: POPULATE_FIELDS,
    //   data: attributes.permissions.map(action => ({ action, token: apiToken })),
    // });
    await Promise.all(
      uniq(attributes.permissions).map((action) =>
        balerion.db.query('admin::api-token-permission').create({
          data: { action, token: apiToken },
        })
      )
    );

    const currentPermissions = await balerion.db
      .query('admin::api-token')
...
const regenerate = async (id: string | number): Promise<ApiToken> => {
  ...
  const apiToken: ApiToken = await balerion.db.query('admin::api-token').update({
    select: ['id', 'accessKey'],
    where: { id },
    data: {
      accessKey: hash(accessKey),
...
const checkSaltIsDefined = () => {
  if (!balerion.config.get('admin.apiToken.salt')) {
    // TODO V5: stop reading API_TOKEN_SALT
    if (process.env.API_TOKEN_SALT) {
      process.emitWarning(`[deprecated] In future versions, Balerion will stop reading directly from the environment variable API_TOKEN_SALT. Please set apiToken.salt in config/admin.js instead.
For security reasons, keep storing the secret in an environment variable and use env() to read it in config/admin.js (ex: \`apiToken: { salt: env('API_TOKEN_SALT') }\`). See https://docs.balerion.io/developer-docs/latest/setup-deployment-guides/configurations/optional/environment.html#configuration-using-environment-variables.`);

      balerion.config.set('admin.apiToken.salt', process.env.API_TOKEN_SALT);
    } else {
      throw new Error(
...
For security reasons, prefer storing the secret in an environment variable and read it in config/admin.js. See https://docs.balerion.io/developer-docs/latest/setup-deployment-guides/configurations/optional/environment.html#configuration-using-environment-variables.`
      );
    }
  }
};
...
const list = async (): Promise<Array<ApiToken>> => {
  const tokens: Array<DBApiToken> = await balerion.db.query('admin::api-token').findMany({
    select: SELECT_FIELDS,
    populate: POPULATE_FIELDS,
    orderBy: { name: 'ASC' },
  });
...
const update = async (
  ...
  const originalToken: DBApiToken = await balerion.db
    .query('admin::api-token')
    .findOne({ where: { id } });
  ...
  const updatedToken: ApiToken = await balerion.db.query('admin::api-token').update({
    select: SELECT_FIELDS,
    where: { id },
  ...
  if (updatedToken.type === constants.API_TOKEN_TYPE.CUSTOM && attributes.permissions) {
    const currentPermissionsResult = await balerion.db
      .query('admin::api-token')
    ...
    await Promise.all(
      actionsToDelete.map((action) =>
        balerion.db.query('admin::api-token-permission').delete({
  ...
  // if type is not custom, make sure any old permissions get removed
    ...
    await balerion.db.query('admin::api-token-permission').delete({
      where: { token: id },
    });
  }

  // retrieve permissions
  const permissionsFromDb = await balerion.db
    .query('admin::api-token')
    .load(updatedToken, 'permissions');
...
const count = async (where = {}): Promise<number> => {
  return balerion.db.query('admin::api-token').count({ where });
};
