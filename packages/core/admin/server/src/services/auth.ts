...
const checkCredentials = async ({ email, password }: { email: string; password: string }) => {
  const user: AdminUser = await balerion.db.query('admin::user').findOne({ where: { email } });
...
const forgotPassword = async ({ email } = {} as { email: string }) => {
  const user: AdminUser = await balerion.db
    .query('admin::user')
  ...
  return balerion
    ...
    .sendTemplatedEmail(
      {
        to: user.email,
        from: balerion.config.get('admin.forgotPassword.from'),
        replyTo: balerion.config.get('admin.forgotPassword.replyTo'),
      },
      balerion.config.get('admin.forgotPassword.emailTemplate'),
      {
        url,
...
const resetPassword = async (
  ...
  const matchingUser: AdminUser | undefined = await balerion.db
    .query('admin::user')
