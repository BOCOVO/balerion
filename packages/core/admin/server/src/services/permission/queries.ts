...
export const deleteByRolesIds = async (rolesIds: Data.ID[]): Promise<void> => {
  const permissionsToDelete = await balerion.db.query('admin::permission').findMany({
    select: ['id'],
    where: {
      role: { id: rolesIds },
    },
...
export const deleteByIds = async (ids: Data.ID[]): Promise<void> => {
  ...
  for (const id of ids) {
    const queryResult = await balerion.db.query('admin::permission').delete({ where: { id } });
    result.push(queryResult);
  }
  balerion.eventHub.emit('permission.delete', { permissions: result });
};
...
export const createMany = async (permissions: CreatePermissionPayload[]): Promise<Permission[]> => {
  ...
  for (const permission of permissions) {
    const newPerm = await balerion.db.query('admin::permission').create({ data: permission });
    createdPermissions.push(newPerm);
  }

  const permissionsToReturn = permissionDomain.toPermission(createdPermissions);
  balerion.eventHub.emit('permission.create', { permissions: permissionsToReturn });
...
const update = async (params: unknown, attributes: Partial<Permission>) => {
  const updatedPermission = (await balerion.db
  ...
  balerion.eventHub.emit('permission.update', { permissions: permissionToReturn });
...
export const findMany = async (params = {}): Promise<Permission[]> => {
  const rawPermissions = await balerion.db.query('admin::permission').findMany(params);
...
export const cleanPermissionsInDatabase = async (): Promise<void> => {
  ...
  const total = await balerion.db.query('admin::permission').count();
  const pageCount = Math.ceil(total / pageSize);

  for (let page = 0; page < pageCount; page += 1) {
    // 1. Find invalid permissions and collect their ID to delete them later
    const results = (await balerion.db
      .query('admin::permission')
