import type { Core, Modules, Schema } from '@balerion/types';
import { contentTypes } from '@balerion/utils';
import type { GetRecentDocuments, RecentDocument } from '../../../shared/contracts/homepage';

const createHomepageService = ({ balerion }: { balerion: Core.Balerion }) => {
  const MAX_DOCUMENTS = 4;

  const metadataService = balerion.plugin('content-manager').service('document-metadata');
  const permissionService = balerion.admin.services.permission as typeof import('./permission');
  ...
  const getConfiguration = async (
    ...
    /**
     * Don't use the balerion.store util because we need to make
     * more precise queries than exact key matches, in order to make as few queries as possible.
     */
    const coreStore = balerion.db.query('balerion::core-store');
  ...
  const getPermittedContentTypes = async () => {
    const readPermissions = await permissionService.findMany({
      where: {
        role: { users: { id: balerion.requestContext.get()?.state?.user.id } },
    ...
    return Promise.all(
      documents.map(async (recentDocument) => {
        const hasDraftAndPublish = contentTypes.hasDraftAndPublish(
          balerion.contentType(recentDocument.contentTypeUid)
        );
        /**
  ...
  const permissionCheckerService = balerion.plugin('content-manager').service('permission-checker');
  const getPermissionChecker = (uid: string) =>
    permissionCheckerService.create({
      userAbility: balerion.requestContext.get()?.state.userAbility,
      model: uid,
    });
  ...
  return {
    async getRecentlyPublishedDocuments(): Promise<GetRecentDocuments.Response['data']> {
      ...
      const allowedContentTypeUids = permittedContentTypes.filter((uid) => {
        return contentTypes.hasDraftAndPublish(balerion.contentType(uid));
      });
      ...
      const recentDocuments = await Promise.all(
        contentTypesMeta.map(async (meta) => {
          ...
          const docs = await balerion.documents(meta.uid).findMany(permissionQuery);
