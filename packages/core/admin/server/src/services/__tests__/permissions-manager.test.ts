...
import { buildBalerionQuery } from '../permission/permissions-manager/query-builders';
...
describe('Permissions Manager', () => {
  ...
  describe('buildBalerionQuery', () => {
    ...
    test.each(tests)(`Test n°%#: %s`, (name, input, expected) => {
      expect(buildBalerionQuery(input)).toStrictEqual(expected);
    });
  });
});
