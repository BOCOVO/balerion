...
import { errors, queryParams } from '@balerion/utils';
...
describe('User', () => {
      ...
      if (name === 'query-params') {
        const transformer = queryParams.createTransformer({
          getModel(name: string) {
            return balerion.getModel(name as any);
          },
        });
  ...
  describe('update', () => {
    test('Hash password', async () => {
      ...
      const id = 1;
      const input = { email: 'test@balerion.io', password: '123' };
    ...
    test('Forwards call to the query layer', async () => {
      ...
      const id = 1;
      const input = { email: 'test@balerion.io' };
      const result = await updateById(id, input);
  ...
  describe('register', () => {
    ...
    test('Calls udpate service', async () => {
      ...
      expect(updateById).toHaveBeenCalledWith(
        1,
        expect.objectContaining({ firstname: 'test', lastname: 'Balerion', password: 'Test1234' })
      );
    });
