...
import type { Modules, Struct } from '@balerion/types';
  ...
  return actions.reduce((permissions, action) => {
    ...
    for (const subject of validSubjects) {
      const fields = actionDomain.appliesToProperty('fields', action)
        ? getNestedFields(balerion.contentTypes[subject], {
            components: balerion.components,
            nestingLevel,
          })
        : undefined;
...
const cleanPermissionFields = (
  ...
  return permissions.map((permission: any) => {
    ...
    if (!subject || !balerion.contentTypes[subject]) {
      return permission;
    }

    const possibleFields = getNestedFieldsWithIntermediate(balerion.contentTypes[subject], {
      components: balerion.components,
      nestingLevel,
    });

    const requiredFields = getNestedFields(balerion.contentTypes[subject], {
      components: balerion.components,
      requiredOnly: true,
      nestingLevel,
