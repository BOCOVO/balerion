...
export default {
  login: compose([
    (ctx: Context, next: Next) => {
      return passport.authenticate('local', { session: false }, (err, user, info) => {
        if (err) {
          balerion.eventHub.emit('admin.auth.error', { error: err, provider: 'local' });
        ...
        if (!user) {
          balerion.eventHub.emit('admin.auth.error', {
        ...
        balerion.eventHub.emit('admin.auth.success', { user: sanitizedUser, provider: 'local' });
  ...
  async registerAdmin(ctx: Context) {
    ...
    balerion.telemetry.send('didCreateFirstAdmin');
  ...
  logout(ctx: Context) {
    ...
    balerion.eventHub.emit('admin.logout', { user: sanitizedUser });
    ctx.body = { data: {} };
  },
};
