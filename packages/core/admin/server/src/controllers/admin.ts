...
export default {
  ...
  async telemetryProperties(ctx: Context) {
    ...
    const useTypescriptOnServer = await isUsingTypeScript(balerion.dirs.app.root);
  ...
  async information() {
    const currentEnvironment: string = balerion.config.get('environment');
    const autoReload = balerion.config.get('autoReload', false);
    const balerionVersion = balerion.config.get('info.balerion', null);
  ...
  async plugins(ctx: Context) {
    const enabledPlugins = balerion.config.get('enabledPlugins') as any;
