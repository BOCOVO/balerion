...
export default {
  ...
  async getWebhook(ctx: Context) {
    const { id } = ctx.params;
    const webhook = await balerion.get('webhookStore').findWebhook(id);
  ...
  async createWebhook(ctx: Context) {
    ...
    const webhook = await balerion.get('webhookStore').createWebhook(body);
  ...
  async updateWebhook(ctx: Context) {
    ...
    const updatedWebhook = await balerion.get('webhookStore').updateWebhook(id, {
      ...webhook,
      ...body,
    });
    ...
    balerion.get('webhookRunner').update(updatedWebhook);
  ...
  async deleteWebhook(ctx: Context) {
    ...
    await balerion.get('webhookStore').deleteWebhook(id);

    balerion.get('webhookRunner').remove(webhook);
