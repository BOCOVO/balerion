...
export const roles = yup.array(yup.balerionID()).min(1);

const isAPluginName = yup
  .string()
  .test('is-a-plugin-name', 'is not a plugin name', function (value) {
    return [undefined, 'admin', ...Object.keys(balerion.plugins)].includes(value)
      ? true
...
export const arrayOfConditionNames = yup
  ...
  .test('is-an-array-of-conditions', 'is not a plugin name', function (value) {
    const ids = balerion.service('admin::permission').conditionProvider.keys();
