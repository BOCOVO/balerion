...
import { contentTypes as contentTypesUtils, errors } from '@balerion/utils';
import type { UID, Struct } from '@balerion/types';
...
export const createContentType = async (
  ...
  balerion.eventHub.emit('content-type.create', { contentType: newContentType });
...
export const generateAPI = ({
  ...
  const balerionGenerators = require('@balerion/generators');
  return balerionGenerators.generate(
    'content-type',
    {
      kind,
      singularName,
...
export const editContentType = async (
  ...
  if (newKind !== previousKind && newKind === 'singleType') {
    const entryCount = await balerion.db.query(uid).count();
    if (entryCount > 1) {
  ...
  if (newKind !== previousKind) {
    const apiHandler = balerion.plugin('content-type-builder').service('api-handler');
  ...
  balerion.eventHub.emit('content-type.update', { contentType: updatedContentType });
...
export const deleteContentType = async (uid: UID.ContentType, defaultBuilder: any = undefined) => {
  ...
  balerion.eventHub.emit('content-type.delete', { contentType });
