...
import { strings, errors } from '@balerion/utils';
  ...
  return {
    ...
    createComponent(this: any, infos: any) {
      ...
      const handler = createSchemaHandler({
        dir: path.join(balerion.dirs.app.components, strings.nameToSlug(infos.category)),
      ...
      if (this.components.size === 0) {
        balerion.telemetry.send('didCreateFirstComponent');
      } else {
        balerion.telemetry.send('didCreateComponent');
      }
    ...
    editComponent(this: any, infos: any) {
      ...
      const newDir = path.join(balerion.dirs.app.components, newCategory);
