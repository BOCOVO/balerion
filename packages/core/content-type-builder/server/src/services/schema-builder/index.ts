...
export default function createBuilder() {
  const components = Object.values(balerion.components).map((componentInput) => ({
    ...
    dir: join(balerion.dirs.app.components, componentInput.category),
    schema: componentInput.__schema__,
    config: componentInput.config,
  }));

  const contentTypes = Object.values<any>(balerion.contentTypes).map((contentTypeInput) => {
  ...
  return {
    ...
    writeFiles() {
      ...
      return Promise.all(schemas.map((schema) => schema.flush()))
        .catch((error) => {
          balerion.log.error('Error writing schema files');
