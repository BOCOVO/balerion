...
export async function clear(uid: Internal.UID.ContentType) {
  ...
  const { apiName, modelName } = balerion.contentTypes[uid] as any;

  const apiFolder = path.join(balerion.dirs.app.api, apiName);
...
export async function backup(uid: Internal.UID.ContentType) {
  const { apiName } = balerion.contentTypes[uid] as any;

  const apiFolder = path.join(balerion.dirs.app.api, apiName);
  const backupFolder = path.join(balerion.dirs.app.api, '.backup', apiName);
...
async function deleteBackup(uid: Internal.UID.ContentType) {
  ...
  const backupFolder = path.join(balerion.dirs.app.api, '.backup');
  const apiBackupFolder = path.join(balerion.dirs.app.api, '.backup', apiName);
