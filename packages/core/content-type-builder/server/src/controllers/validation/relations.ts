...
const BALERION_USER_RELATIONS = ['oneToOne', 'oneToMany'];
...
const isValidRelation = (validNatures: ReadonlyArray<string>): TestFunction<string | undefined> =>
  function (this: TestContext, value) {
    ...
    if (this.parent.target === coreUids.BALERION_USER) {
      if (!validNatures.includes(value) || !isUndefined(this.parent.targetAttribute)) {
        return this.createError({
          path: this.path,
          message: `must be one of the following values: ${BALERION_USER_RELATIONS.join(', ')}`,
        });
      }
    }
...
export const getRelationValidator = (
  ...
  const contentTypesUIDs = Object.keys(balerion.contentTypes)
    .filter((key) => balerion.contentTypes[key as UID.ContentType].kind === typeKinds.COLLECTION_TYPE)
    .filter((key) => !key.startsWith(coreUids.PREFIX) || key === coreUids.BALERION_USER)
