...
export default ({ balerion }: { balerion: Core.Balerion }) => {
  ...
  const stagePermissionsService = getService('stage-permissions', { balerion });
  const workflowValidator = getService('validation', { balerion });

  return {
    find({ workflowId, populate }: any) {
      return balerion.db.query(STAGE_MODEL_UID).findMany({
    ...
    findById(id: string | number, { populate }: any = {}) {
      return balerion.db.query(STAGE_MODEL_UID).findOne({
        where: { id },
        populate,
      });
    },
    ...
    async createMany(stagesList: any, { fields }: any = {}) {
      ...
      const stages = await Promise.all(
        stagesList.map((stage: any) =>
          balerion.db.query(STAGE_MODEL_UID).create({
    ...
    async update(srcStage: any, destStage: any) {
      ...
      const stage = await balerion.db.query(STAGE_MODEL_UID).update({
        where: { id: stageId },
        data: {
          ...destStage,
    ...
    async delete(stage: any) {
      ...
      const deletedStage = await balerion.db.query(STAGE_MODEL_UID).delete({
        where: { id: stage.id },
      });
    ...
    async deleteMany(stages: any) {
      ...
      return balerion.db.query(STAGE_MODEL_UID).deleteMany({
    ...
    count({ workflowId }: any = {}) {
      ...
      return balerion.db.query(STAGE_MODEL_UID).count(opts);
    },
    ...
    async replaceStages(srcStages: any, destStages: any, contentTypesToMigrate = []) {
      ...
      return balerion.db.transaction(async ({ trx }) => {
    ...
    async updateEntity(
      ...
      const entity = await balerion.documents(model).update({
        documentId,
        locale,
      ...
      const { tableName } = balerion.db.metadata.get(model);
      await balerion.db
    ...
    async updateEntitiesStage(contentTypeUID: any, { fromStageId, toStageId }: any) {
      ...
      return balerion.db.transaction(async ({ trx }) => {
        ...
        const selectStatement = balerion.db
          .getConnection()
    ...
    async deleteAllEntitiesStage(contentTypeUID: any) {
      const { attributes } = balerion.db.metadata.get(contentTypeUID) as any;
      const joinTable = attributes[ENTITY_STAGE_ATTRIBUTE].joinTable;

      // Delete all stage links for the content type
      return balerion.db.transaction(async ({ trx }) =>
        balerion.db.getConnection().from(joinTable.name).delete().transacting(trx)
      );
    },
  };
};
