...
const processFilters = ({ balerion }: { balerion: Core.Balerion }, filters: any = {}) => {
  const processedFilters = { ...filters };

  if (isString(filters.contentTypes)) {
    processedFilters.contentTypes = getWorkflowContentTypeFilter({ balerion }, filters.contentTypes);
  }
...
export default ({ balerion }: { balerion: Core.Balerion }) => {
  const workflowsContentTypes = workflowsContentTypesFactory({ balerion });
  const workflowValidator = getService('validation', { balerion });
  const metrics = getService('workflow-metrics', { balerion });
  ...
  return {
     ...
     */
    async find(opts: any = {}) {
      const filters = processFilters({ balerion }, opts.filters);
      const populate = processPopulate(opts.populate);

      const query = balerion.get('query-params').transform(WORKFLOW_MODEL_UID, {
        ...opts,
        filters,
        populate,
      });

      return balerion.db.query(WORKFLOW_MODEL_UID).findMany(query);
    },
    ...
    findById(id: any, opts: { populate?: any } = {}) {
      ...
      const query = balerion.get('query-params').transform(WORKFLOW_MODEL_UID, { populate });

      return balerion.db.query(WORKFLOW_MODEL_UID).findOne({
        ...query,
        where: { id },
      });
    },
    ...
    async create(opts: { data: any }) {
      ...
      return balerion.db.transaction(async () => {
        // Create stages
        const stages = await getService('stages', { balerion }).createMany(opts.data.stages);
        ...
        // Create Workflow
        const createdWorkflow = await balerion.db
          .query(WORKFLOW_MODEL_UID)
          .create(balerion.get('query-params').transform(WORKFLOW_MODEL_UID, createOpts));
    ...
    async update(workflow: any, opts: any) {
      const stageService = getService('stages', { balerion });
      ...
      return balerion.db.transaction(async () => {
        ...
        const query = balerion.get('query-params').transform(WORKFLOW_MODEL_UID, updateOpts);

        // Update Workflow
        const updatedWorkflow = await balerion.db.query(WORKFLOW_MODEL_UID).update({
          ...query,
    ...
    async delete(workflow: any, opts: any) {
      ...
      return balerion.db.transaction(async () => {
        ...
        const query = balerion.get('query-params').transform(WORKFLOW_MODEL_UID, opts);

        // Delete Workflow
        const deletedWorkflow = await balerion.db.query(WORKFLOW_MODEL_UID).delete({
          ...query,
          where: { id: workflow.id },
        });
     ...
     */
    count() {
      return balerion.db.query(WORKFLOW_MODEL_UID).count();
    },
    ...
    async _getAssignedWorkflows(uid: any, opts = {}) {
      return this.find({
        ...opts,
        filters: { contentTypes: getWorkflowContentTypeFilter({ balerion }, uid) },
      });
    },
