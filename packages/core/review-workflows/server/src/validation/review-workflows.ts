...
const validateContentTypes = yup.array().of(
  yup
    ...
    .test({
      ...
      test(uid: any) {
        // Warning; we use the balerion global - to avoid that, it would need to refactor how
        // we generate validation function by using a factory with the balerion instance as parameter.
        return !!balerion.getModel(uid);
      },
    })
    .test({
