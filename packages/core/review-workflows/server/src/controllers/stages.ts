...
import { async, validate } from '@balerion/utils';
...
/**
 *
 * @param { Core.Balerion } balerion - Balerion instance
 * @param userAbility
 * @return { (Stage) => SanitizedStage }
 */
function sanitizeStage({ balerion }: { balerion: Core.Balerion }, userAbility: unknown) {
...
export default {
  ...
  async find(ctx: Context) {
    ...
    const sanitizer = sanitizeStage({ balerion }, ctx.state.userAbility);
  ...
  async updateEntity(ctx: Context) {
    ...
    const entity = await balerion.documents(modelUID).findOne({
      documentId,
