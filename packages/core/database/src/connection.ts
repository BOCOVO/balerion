...
export const createConnection = (userConfig: Knex.Config, balerionConfig?: Partial<Knex.Config>) => {
  ...
  if (balerionConfig?.pool?.afterCreate) {
    ...
    const balerionAfterCreate = balerionConfig.pool.afterCreate;
    knexConfig.pool.afterCreate = (
      conn: unknown,
      done: (err: Error | null | undefined, connection: any) => void
    ) => {
      balerionAfterCreate(conn, (err: Error | null | undefined, nativeConn: any) => {
        if (err) {
