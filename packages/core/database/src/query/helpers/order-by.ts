...
const COL_BALERION_ROW_NUMBER = '__balerion_row_number';
const COL_BALERION_ORDER_BY_PREFIX = '__balerion_order_by';
...
export const getBalerionOrderColumnAlias = (column: string) => {
  const trimmedColumnName = column.replaceAll('.', '_');

  return `${COL_BALERION_ORDER_BY_PREFIX}__${trimmedColumnName}`;
};
...
export const wrapWithDeepSort = (originalQuery: knex.Knex.QueryBuilder, ctx: OrderByCtx) => {
  ...
  baseQuery.select(
    ...
    //      Then, "t2.name" will become "t2.name as __balerion_order_by__t2_name"
    ...orderBy.map((orderByClause) =>
      alias(getBalerionOrderColumnAlias(orderByClause.column), orderByClause.column)
    )
  );
  ...
  const selectRowsAsNumberedPartitions = (partitionedQuery: knex.Knex.QueryBuilder) => {
    ...
    const prefixedOrderBy = orderBy.map((orderByClause) => ({
      column: prefix(baseQueryAlias, getBalerionOrderColumnAlias(orderByClause.column)),
    ...
    partitionedQuery
      ...
      .rowNumber(COL_BALERION_ROW_NUMBER, (subQuery) => {
  ...
  resultQuery
    ...
    .innerJoin(selectRowsAsNumberedPartitions, function () {
      this
        ...
        .andOnVal(`${partitionedQueryAlias}.${COL_BALERION_ROW_NUMBER}`, '=', 1);
    });
  ...
  resultQuery.orderBy([
    ...
    ...orderBy.map((orderByClause) => ({
      column: prefix(partitionedQueryAlias, getBalerionOrderColumnAlias(orderByClause.column)),
      order: orderByClause.order,
    })),
