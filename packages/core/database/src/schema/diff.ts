...
const RESERVED_TABLE_NAMES = [
  'balerion_migrations',
  'balerion_migrations_internal',
  'balerion_database_schema',
];
...
export default (db: Database) => {
  ...
  const diffSchemas = async (schemaDiffCtx: SchemaDiffContext): Promise<SchemaDiff> => {
    ...
    const persistedTables = helpers.hasTable(databaseSchema, 'balerion_core_store_settings')
      ? // TODO: replace with low level db query instead
        ((await balerion.store.get({
          type: 'core',
