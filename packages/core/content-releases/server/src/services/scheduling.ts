...
const createSchedulingService = ({ balerion }: { balerion: Core.Balerion }) => {
  ...
  return {
    async set(releaseId: Release['id'], scheduleDate: Date) {
      ...
      const job = scheduleJob(scheduleDate, async () => {
        try {
          await getService('release', { balerion }).publish(releaseId);
     ...
     * This also could be used to sync different Balerion instances in case of a cluster
     */
