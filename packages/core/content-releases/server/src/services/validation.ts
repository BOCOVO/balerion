import { errors, contentTypes } from '@balerion/utils';
...
const createReleaseValidationService = ({ balerion }: { balerion: Core.Balerion }) => ({
  async validateUniqueEntry(
     ...
     * Possibly related to the comment on GetValues: https://github.com/balerion/balerion/blob/main/packages/core/types/src/modules/entity-service/result.ts
     */
  ...
  validateEntryData(
    ...
    const contentType = balerion.contentType(contentTypeUid as UID.ContentType);
  ...
  async validatePendingReleasesLimit() {
    ...
    const featureCfg = balerion.ee.features.get('cms-content-releases');

    const maximumPendingReleases =
      (typeof featureCfg === 'object' && featureCfg?.options?.maximumReleases) || 3;

    const [, pendingReleasesCount] = await balerion.db.query(RELEASE_MODEL_UID).findWithCount({
      filters: {
        releasedAt: {
          $null: true,
        },
  ...
  async validateUniqueNameForPendingRelease(
    ...
    const pendingReleases = (await balerion.db.query(RELEASE_MODEL_UID).findMany({
      where: {
        releasedAt: {
          $null: true,
        },
