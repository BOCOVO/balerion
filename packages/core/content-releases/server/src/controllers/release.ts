...
const releaseController = {
  ...
  async findByDocumentAttached(ctx: Koa.Context) {
    const permissionsManager = balerion.service('admin::permission').createPermissionsManager({
      ability: ctx.state.userAbility,
      model: RELEASE_MODEL_UID,
    });
    await permissionsManager.validateQuery(ctx.query);
    const releaseService = getService('release', { balerion });
    const query = await permissionsManager.sanitizeQuery(ctx.query);

    await validatefindByDocumentAttachedParams(query);

    // If entry is a singleType, we need to manually add the entryDocumentId to the query
    const model = balerion.getModel(query.contentType) as Schema.ContentType;
  ...
  async findPage(ctx: Koa.Context) {
    ...
    const pendingReleasesCount = await balerion.db.query(RELEASE_MODEL_UID).count({
      where: {
        releasedAt: null,
      },
    });
  ...
  async findOne(ctx: Koa.Context) {
    ...
    const sanitizedRelease = {
      ...
      createdBy: release.createdBy
        ? balerion.service('admin::user').sanitizeUser(release.createdBy)
        : null,
    };
