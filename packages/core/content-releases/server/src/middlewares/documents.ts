import type { Modules, UID } from '@balerion/types';
...
const updateActionsStatusAndUpdateReleaseStatus = async (
  ...
  const entryStatus = await isEntryValid(contentType, entry, { balerion });

  await balerion.db.query(RELEASE_ACTION_MODEL_UID).updateMany({
    where: {
      contentType,
      entryDocumentId: entry.documentId,
      locale: entry.locale,
...
const deleteActionsOnDelete: Middleware = async (ctx, next) => {
  ...
  try {
    ...
    balerion.log.error('Error while deleting release actions after delete', {
      error,
    });
  }
...
const updateActionsOnUpdate: Middleware = async (ctx, next) => {
  ...
  try {
    ...
    balerion.log.error('Error while updating release actions after update', {
      error,
    });
  }
