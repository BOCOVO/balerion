...
import { contentTypes as contentTypesUtils, async } from '@balerion/utils';
...
export async function migrateIsValidAndStatusReleases() {
  const releasesWithoutStatus = (await balerion.db.query(RELEASE_MODEL_UID).findMany({
    where: {
      status: null,
      releasedAt: null,
    },
  ...
  async.map(releasesWithoutStatus, async (release: Release) => {
    ...
    return getService('release', { balerion }).updateReleaseStatus(release.id);
  });

  const publishedReleases = await balerion.db.query(RELEASE_MODEL_UID).findMany({
    where: {
      status: null,
      releasedAt: {
        $notNull: true,
    ...
    async
      ...
      .then(() => {
        ...
        async.map(releasesAffected, async (releaseId: Release['id']) => {
          return getService('release', { balerion }).updateReleaseStatus(releaseId);
        });
      });
  }
}
...
export async function disableContentTypeLocalized({ oldContentTypes, contentTypes }: Input) {
  ...
  const i18nPlugin = balerion.plugin('i18n');
  if (!i18nPlugin) {
    return;
  }
