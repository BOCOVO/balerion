...
/**
 * Utility to extend Balerion configuration middlewares. Mainly used to extend the CSP directives from the security middleware.
 */
const extendMiddlewareConfiguration = (middleware = { name: '', config: {} }) => {
  const middlewares = balerion.config.get('middlewares') as (string | object)[];
  ...
  balerion.config.set('middlewares', configuredMiddlewares);
};

/**
 * Read configuration for static preview
 */
const createPreviewConfigService = ({ balerion }: { balerion: Core.Balerion }) => {
  return {
    register() {
      if (!this.isEnabled()) {
        return;
      ...
      const config = balerion.config.get('admin.preview') as PreviewConfig;
