...
import type { Core, Modules, Schema, Data, Struct, UID } from '@balerion/types';
...
export const createServiceUtils = ({ balerion }: { balerion: Core.Balerion }) => {
  /**
   * @description
    ...
    if (Array.isArray(versionRelationData)) {
      ...
      const existingAndMissingRelations = await Promise.all(
        versionRelationData.map((relation) => {
          return balerion.documents(attribute.target).findOne({
    ...
    if (Array.isArray(versionRelationData)) {
      const existingAndMissingMedias = await Promise.all(
        versionRelationData.map((media) => {
          return balerion.db.query('plugin::upload.file').findOne({ where: { id: media.id } });
        })
      );
  ...
  const localesService = balerion.plugin('i18n')?.service('locales');
  const i18nContentTypeService = balerion.plugin('i18n')?.service('content-types');
  ...
  const getRetentionDays = () => {
    const featureConfig = balerion.ee.features.get('cms-content-history');
    const licenseRetentionDays =
      typeof featureConfig === 'object' && featureConfig?.options.retentionDays;
    const userRetentionDays: number = balerion.config.get('admin.history.retentionDays');
  ...
  const getVersionStatus = async (
    ...
    const documentMetadataService = balerion.plugin('content-manager').service('document-metadata');
  ...
  const getComponentFields = (componentUID: UID.Component): string[] => {
    return Object.entries(balerion.getModel(componentUID).attributes).reduce<string[]>(
      ...
      values
        ...
        .reduce(
          async (currentRelationDataPromise, entry) => {
            ...
            const relatedEntry = await balerion.db
              .query('plugin::upload.file')
      ...
      values
        ...
        .reduce(
          async (currentRelationDataPromise, entry) => {
            ...
            const relatedEntry = await balerion
