...
const shouldCreateHistoryVersion = (
  ...
  if (!balerion.requestContext.get()?.request.url.startsWith('/content-manager')) {
    return false;
  }
  ...
  if (
    context.action === 'update' &&
    balerion.requestContext.get()?.request.url.endsWith('/actions/publish')
  ) {
    return false;
  }
...
const getSchemas = (uid: UID.CollectionType) => {
  const attributesSchema = balerion.getModel(uid).attributes;
  ...
  const componentsSchemas = Object.keys(attributesSchema).reduce(
    (currentComponentSchemas, key) => {
      ...
      if (fieldSchema.type === 'component') {
        const componentSchema = balerion.getModel(fieldSchema.component).attributes;
        return {
...
const createLifecyclesService = ({ balerion }: { balerion: Core.Balerion }) => {
  const state: {
  ...
  const serviceUtils = createServiceUtils({ balerion });
  ...
  return {
    async bootstrap() {
      ...
      balerion.documents.use(async (context, next) => {
        ...
        const localeEntries = await balerion.db.query(uid).findMany({
          where: {
            documentId,
            ...(isLocalizedContentType ? { locale: { $in: locales } } : {}),
            ...(contentTypes.hasDraftAndPublish(balerion.contentTypes[uid])
              ? { publishedAt: null }
              : {}),
          },
        ...
        await balerion.db.transaction(async ({ onCommit }) => {
          ...
          onCommit(async () => {
            for (const entry of localeEntries) {
              ...
              await getService(balerion, 'history').createVersion({
                contentType: uid,
        ...
        balerion.db
          ...
          .catch((error) => {
            if (error instanceof Error) {
              balerion.log.error('Error deleting expired history versions', error.message);
            }
          });
      });
