import type { Core, Data, Modules, Schema } from '@balerion/types';
import { errors, traverseEntity } from '@balerion/utils';
...
const createHistoryService = ({ balerion }: { balerion: Core.Balerion }) => {
  const query = balerion.db.query(HISTORY_VERSION_UID);
  ...
  return {
    async createVersion(historyVersionData: HistoryVersions.CreateHistoryVersion) {
      await query.create({
        data: {
          ...historyVersionData,
          createdAt: new Date(),
          createdBy: balerion.requestContext.get()?.state?.user.id,
        },
      });
    },
    ...
    async findVersionsPage(params: HistoryVersions.GetHistoryVersions.Request): Promise<{
      ...
      const schema = balerion.getModel(params.query.contentType);
              ...
              if (options.attribute.target === 'admin::user') {
                const adminUsers = await Promise.all(
                  currentValue.map((userToPopulate) => {
                    ...
                    return balerion.query('admin::user').findOne({
                      where: {
          ...
          return {
            ...
            meta: {
              unknownAttributes: serviceUtils.getSchemaAttributesDiff(
                result.schema,
                balerion.getModel(params.query.contentType).attributes
              ),
            },
    ...
    async restoreVersion(versionId: Data.ID) {
      ...
      const contentTypeSchemaAttributes = balerion.getModel(version.contentType).attributes;
      ...
      const restoredDocument = await balerion.documents(version.contentType).update({
