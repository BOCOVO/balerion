...
import { setCreatorFields, async, errors } from '@balerion/utils';
...
export default {
  ...
  async create(ctx: any) {
    ...
    if (totalEntries === 0) {
      balerion.telemetry.send('didCreateFirstContentTypeEntry', {
  ...
  async publish(ctx: any) {
    ...
    const publishedDocument = await balerion.db.transaction(async () => {
