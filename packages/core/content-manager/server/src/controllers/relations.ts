...
import { isOperatorOfType, contentTypes, relations, errors } from '@balerion/utils';
import type { Data, Modules, UID } from '@balerion/types';
...
const addStatusToRelations = async (targetUid: UID.Schema, relations: RelationEntity[]) => {
  if (!contentTypes.hasDraftAndPublish(balerion.getModel(targetUid))) {
    return relations;
  }
  ...
  const availableStatus = await balerion.query(targetUid).findMany({
...
const validateLocale = (sourceUid: UID.Schema, targetUid: UID.ContentType, locale?: string) => {
  const sourceModel = balerion.getModel(sourceUid);
  const targetModel = balerion.getModel(targetUid);

  const isLocalized = balerion.plugin('i18n').service('content-types').isLocalizedContentType;
...
export default {
  async extractAndValidateRequestInfo(ctx: any, id?: Data.ID) {
    ...
    const sourceSchema = balerion.getModel(model);
    if (!sourceSchema) {
    ...
    if (id) {
      ...
      const currentEntity = await balerion.db.query(model).findOne({
        where,
        populate,
      });
    ...
    const targetSchema = balerion.getModel(targetUid);
  ...
  async findAvailable(ctx: any) {
    ...
    if (id) {
       ...
       */
      const subQuery = balerion.db.queryBuilder(sourceUid);
    ...
    const dbQuery = balerion.get('query-params').transform(targetUid, queryParams);

    const res = await balerion.db.query(targetUid).findPage(dbQuery);
  ...
  async findExisting(ctx: any) {
     ...
     */
    const dbQuery = balerion.db.query(sourceUid);
    ...
    const sanitizedRes = await loadRelations({ id: entryId }, targetField, {
      ...balerion.get('query-params').transform(targetUid, permissionQuery),
      ordering: 'desc',
    });
