...
import { yup, validateYupSchema, errors } from '@balerion/utils';
...
const bulkActionInputSchema = yup
  .object({
    documentIds: yup.array().of(yup.balerionID()).min(1).required(),
  })
  .required();
...
const validateUIDField = (contentTypeUID: any, field: any) => {
  const model = balerion.contentTypes[contentTypeUID];
