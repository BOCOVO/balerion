...
import type { UID, Schema, Modules } from '@balerion/types';
import { getService } from '../../utils';

const { isVisibleAttribute, isScalarAttribute, getDoesAttributeRequireValidation } =
  balerionUtils.contentTypes;
const { isAnyToMany } = balerionUtils.relations;
const { PUBLISHED_AT_ATTRIBUTE } = balerionUtils.contentTypes.constants;
...
// TODO: Import from @balerion/types when it's available there
...
/**
 *  Create a Balerion populate object which populates all attribute fields of a Balerion query.
 */
const getQueryPopulate = async (uid: UID.Schema, query: object): Promise<Populate> => {
  let populateQuery: Populate = {};

  await balerionUtils.traverse.traverseQueryFilters(
    /**
     *
     * @param {Object} param0
    ...
    { schema: balerion.getModel(uid), getModel: balerion.getModel.bind(balerion) },
    query
  );
