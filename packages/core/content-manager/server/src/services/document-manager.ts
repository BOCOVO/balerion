...
import { contentTypes, errors, pagination } from '@balerion/utils';
...
const documentManager = ({ balerion }: { balerion: Core.Balerion }) => {
  return {
    async findOne(
      id: string,
      ...
      return balerion.documents(uid).findOne({ ...opts, documentId: id });
    },
    ...
    async findLocales(
      ...
      return balerion.db.query(uid).findMany({ populate: opts.populate, where });
    },
    ...
    async findMany(opts: DocServiceParams<'findMany'>, uid: UID.CollectionType) {
      ...
      return balerion.documents(uid).findMany(params);
    },
    ...
    async findPage(opts: DocServiceParams<'findMany'>, uid: UID.CollectionType) {
      ...
      const [documents, total = 0] = await Promise.all([
        balerion.documents(uid).findMany(params),
        balerion.documents(uid).count(params),
      ]);
    ...
    async create(uid: UID.CollectionType, opts: DocServiceParams<'create'> = {} as any) {
      ...
      return balerion.documents(uid).create(params);
    },
    ...
    async update(
      ...
      return balerion.documents(uid).update({ ...params, documentId: id });
    },
    ...
    async exists(uid: UID.CollectionType, id?: string) {
      // Collection type
      if (id) {
        const count = await balerion.db.query(uid).count({ where: { documentId: id } });
        return count > 0;
      }

      // Single type
      const count = await balerion.db.query(uid).count();
      return count > 0;
    },
    ...
    async delete(
      ...
      await balerion.documents(uid).delete({
        ...opts,
        documentId: id,
        populate,
      });
    ...
    async deleteMany(
      ...
      const deletedEntries = await balerion.db.transaction(async () => {
    ...
    async unpublishMany(
      ...
      const unpublishedEntries = await balerion.db.transaction(async () => {
        return Promise.all(
          documentIds.map((id) =>
            balerion
              .documents(uid)
    ...
    async countDraftRelations(id: string, uid: UID.ContentType, locale: string) {
      ...
      const document = await balerion.documents(uid).findOne({ documentId: id, populate, locale });
      if (!document) {
    ...
    async countManyEntriesDraftRelations(
      ...
      const entities = await balerion.db.query(uid).findMany({
        populate,
        where: {
