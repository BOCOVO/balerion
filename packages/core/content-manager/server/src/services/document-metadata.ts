...
import { async, contentTypes } from '@balerion/utils';
...
export default ({ balerion }: { balerion: Core.Balerion }) => ({
  ...
  async getManyAvailableStatus(uid: UID.ContentType, documents: DocumentVersion[]) {
    ...
    return balerion.query(uid).findMany({
      where,
  ...
  async getMetadata(
    ...
    const dbParams = balerion.get('query-params').transform(uid, params);
    const versions = await balerion.db.query(uid).findMany(dbParams);
  ...
  async formatDocumentWithMetadata(
    ...
    const hasDraftAndPublish = contentTypes.hasDraftAndPublish(balerion.getModel(uid));
