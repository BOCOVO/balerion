...
import type { Core, Schema, UID } from '@balerion/types';
...
export default ({ balerion }: { balerion: Core.Balerion }) => ({
  async generateUIDField({
    ...
    const contentType = balerion.contentTypes[contentTypeUID];
  ...
  async findUniqueUID({
    ...
    const foundDocuments = await balerion.documents(contentTypeUID).findMany({
      filters: {
  ...
  async checkUIDAvailability({
    ...
    const documentCount = await balerion.documents(contentTypeUID).count({
      filters: {
        [field]: value,
      },
      locale,
