...
import { BalerionAppContextValue, useBalerionApp } from '@balerion/admin/balerion-admin';

const componentStore = new Map<string, ComponentType | undefined>();

type LazyComponentStore = Record<string, ComponentType | undefined>;

type CustomField = NonNullable<ReturnType<BalerionAppContextValue['customFields']['get']>>;
...
const useLazyComponents = (componentUids: string[] = []): UseLazyComponentsReturn => {
  ...
  const getCustomField = useBalerionApp('useLazyComponents', (state) => state.customFields.get);
