...
import {
  ...
  useBalerionApp,
  useQueryParams,
} from '@balerion/admin/balerion-admin';
...
const TableActions = ({ document }: TableActionsProps) => {
  ...
  const plugins = useBalerionApp('TableActions', (state) => state.plugins);
...
const EditAction: DocumentActionComponent = ({ documentId }) => {
  ...
  return {
    ...
    onClick: async () => {
      if (!documentId) {
        console.error(
          "You're trying to edit a document without an id, this is likely a bug with Balerion. Please open an issue."
        );
...
const CloneAction: DocumentActionComponent = ({ model, documentId }) => {
  ...
  return {
    ...
    onClick: async () => {
      if (!documentId) {
        console.error(
          "You're trying to clone a document in the table without an id, this is likely a bug with Balerion. Please open an issue."
        );
