...
import { contentTypes as contentTypesUtils, async, errors } from '@balerion/utils';
import type { Modules, UID, Data, Utils, Schema, Core } from '@balerion/types';
...
const isDialectMySQL = () => balerion.db?.dialect.client === 'mysql';
...
>(
  ...
  const { attributes = {} } = balerion.getModel(uid);
    ...
    if (attribute.type === 'component') {
      ...
      if (repeatable === true) {
        ...
        const components = (await async.map(
          ...
          { concurrency: isDialectMySQL() && !balerion.db?.inTransaction() ? 1 : Infinity }
...
const getComponents = async <TUID extends UID.Schema>(
  ...
  const componentAttributes = contentTypesUtils.getComponentAttributes(balerion.getModel(uid));

  if (_.isEmpty(componentAttributes)) {
    return {} as LoadedComponents<TUID>;
  }

  return balerion.db.query(uid).load(entity, componentAttributes) as Promise<LoadedComponents<TUID>>;
};
...
const deleteOldComponents = async <TUID extends UID.Schema>(
  ...
  const previousValue = (await balerion.db
    .query(uid)
...
const deleteComponents = async <TUID extends UID.Schema, TEntity extends Data.Entity<TUID>>(
  ...
  for (const attributeName of attributeNames) {
    ...
    if (attribute.type === 'component' || attribute.type === 'dynamiczone') {
      let value;
      if (loadComponents) {
        value = await balerion.db.query(uid).load(entityToDelete, attributeName);
      } else {
...
const createComponent = async <TUID extends UID.Component = UID.Component>(
  ...
  const model = balerion.getModel(uid) as Schema.Component;
  ...
  return balerion.db.query(uid).create({ data: transform(data) });
};
...
const updateComponent = async <TUID extends UID.Component>(
  ...
  return balerion.db.query(uid).update({
    where: {
...
const deleteComponent = async <TUID extends UID.Component>(
  ...
  await balerion.db.query(uid).delete({ where: { id: componentToDelete.id } });
};
...
const resolveComponentUID = ({
  ...
  for (const path of paths) {
    ...
    if (path in cType.attributes) {
      ...
      if (attribute.type === 'component') {
        cType = balerion.getModel(attribute.component);
      }

      if (attribute.type === 'dynamiczone') {
        cType = ({ __component }: { __component: UID.Component }) => balerion.getModel(__component);
      }
    }
  }
