...
import type { Model } from '@balerion/database';
...
export const deleteRecords = async (balerion: Core.Balerion, options: IRestoreOptions) => {
  const entities = await deleteEntitiesRecords(balerion, options);
  const configuration = await deleteConfigurationRecords(balerion, options);
...
const deleteEntitiesRecords = async (
  ...
  const models = balerion.get('models').get() as Model[];
  const contentTypes = Object.values(balerion.contentTypes) as Struct.ContentTypeSchema[];
  ...
  const contentTypeQuery = queries.entity.createEntityQuery(balerion);
  ...
  const modelsPromises = modelsToClear.map(async (uid) => {
    const result = await balerion.db.query(uid).deleteMany({});
...
const deleteConfigurationRecords = async (
  ...
  if (coreStore) {
    models.push('balerion::core-store');
  }

  if (webhook) {
    models.push('balerion::webhook');
  }
