...
export const createEntitiesWriteStream = (options: IEntitiesRestoreStreamOptions) => {
  const { balerion, updateMappingTable, transaction } = options;
  const query = queries.entity.createEntityQuery(balerion);
  ...
  return new Writable({
    ...
    async write(entity: IEntity, _encoding, callback) {
      await transaction?.attach(async () => {
        ...
        const contentType = balerion.getModel(type);
        ...
        try {
          ...
          diffs.forEach((diff) => {
            if (diff.kind === 'modified' && last(diff.path) === 'id' && 'kind' in contentType) {
              const target = resolveComponentUID({ paths: diff.path, data, contentType, balerion });
