import { createLocalBalerionDestinationProvider } from '../index';
...
const balerionCommonProperties = {
  config: {
    get(service) {
...
describe('Local Balerion Source Destination', () => {
  describe('Bootstrap', () => {
    test('Should not have a defined Balerion instance if bootstrap has not been called', () => {
      const provider = createLocalBalerionDestinationProvider({
  ...
  describe('Strategy', () => {
    test('requires strategy to be restore', async () => {
      const restoreProvider = createLocalBalerionDestinationProvider({
        getBalerion: getBalerionFactory({
          db: {
            transaction,
            lifecycles: {
      ...
      expect(restoreProvider.balerion).toBeDefined();

      await expect(
        (async () => {
          const invalidProvider = createLocalBalerionDestinationProvider({
            getBalerion: getBalerionFactory({
              db: {
                transaction,
                lifecycles: {
    ...
    test('Should delete all entities if it is a restore with only exclude property', async () => {
      ...
      const balerion = getBalerionFactory({
