...
import { ILocalBalerionSourceProviderOptions } from '../local-source';
import { createDispatcher, connectToWebsocket, trimTrailingSlash } from '../utils';

export interface IRemoteBalerionSourceProviderOptions extends ILocalBalerionSourceProviderOptions {
...
class RemoteBalerionSourceProvider implements ISourceProvider {
  name = 'source::remote-balerion';

  type: ProviderType = 'source';

  options: IRemoteBalerionSourceProviderOptions;

  ws: WebSocket | null;

  dispatcher: ReturnType<typeof createDispatcher> | null;

  defaultOptions: Partial<IRemoteBalerionSourceProviderOptions> = {
    streamTimeout: 15000,
  };

  constructor(options: IRemoteBalerionSourceProviderOptions) {
    this.options = {
      ...this.defaultOptions,
      ...options,
    };
...
export const createRemoteBalerionSourceProvider = (options: IRemoteBalerionSourceProviderOptions) => {
  return new RemoteBalerionSourceProvider(options);
};
