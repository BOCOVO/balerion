...
const hasHttpServer = () => {
  // during server restarts, balerion may not have ever been defined at all, so we have to check it first
  return typeof balerion !== 'undefined' && !!balerion?.server?.httpServer;
};
...
const disableTimeouts = () => {
  ...
  const { httpServer } = balerion.server;
  ...
  balerion.log.info('[Data transfer] Disabling http timeouts');
};
const resetTimeouts = () => {
  ...
  balerion.log.info('[Data transfer] Restoring http timeouts');
...
export const handleWSUpgrade = (wss: WebSocketServer, ctx: Context, callback: WSCallback) => {
  ...
  wss.handleUpgrade(ctx.req, ctx.request.socket, Buffer.alloc(0), (client, request) => {
    ...
    disableTimeouts();
    balerion.db.lifecycles.disable();
    balerion.log.info('[Data transfer] Disabling lifecycle hooks');
  ...
  (options: HandlerOptions) => {
    ...
    return async (ctx: Context) => {
      const cb: WSCallback = (ws) => {
        ...
        const cannotRespondHandler = (err: unknown) => {
          ...
          try {
            ...
            balerion?.log?.error('[Data transfer] Failed to close socket on error');
          }
        };
        ...
        ws.on('close', async (...args) => {
          try {
            ...
            balerion?.log?.error('[Data transfer] Uncaught error closing connection');
            balerion?.log?.error(err);
            cannotRespondHandler(err);
          } finally {
            resetTimeouts();
            balerion.db.lifecycles.enable();
            balerion.log.info('[Data transfer] Restoring lifecycle hooks');
          }
        });
        ws.on('error', async (...args) => {
          try {
            ...
            balerion?.log?.error('[Data transfer] Uncaught error in error handling');
            balerion?.log?.error(err);
            cannotRespondHandler(err);
          }
        });
        ws.on('message', async (...args) => {
          try {
            ...
            balerion?.log?.error('[Data transfer] Uncaught error in message handling');
            balerion?.log?.error(err);
            cannotRespondHandler(err);
          }
        });
      ...
      try {
        ...
        balerion?.log?.error('[Data transfer] Error in websocket upgrade request');
        balerion?.log?.error(err);
      }
    };
  };
