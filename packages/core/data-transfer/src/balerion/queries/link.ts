...
export const createLinkQuery = (balerion: Core.Balerion, trx?: Knex.Transaction) => {
  const query = () => {
    const { connection } = balerion.db;
    ...
    async function* generateAllForAttribute(uid: string, fieldName: string): AsyncGenerator<ILink> {
      const metadata = balerion.db.metadata.get(uid);
    ...
    const insert = async (link: ILink) => {
      ...
      const metadata = balerion.db.metadata.get(left.type);
