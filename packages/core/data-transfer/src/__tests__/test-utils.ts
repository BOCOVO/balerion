...
/**
 * Create a "Balerion" like object factory based on the
 * given params and cast it to the correct type
 */
export const getBalerionFactory =
  <
    T extends {
      [key in keyof Partial<Core.Balerion>]: unknown;
    },
  >(
    properties?: T
  ) =>
  (additionalProperties?: Partial<T>) => {
    return { ...properties, ...additionalProperties } as Core.Balerion;
  };
...
/**
 * Factory to get default balerion models test values
 */
export const getBalerionModels = () => {
...
/**
 * Update the global store with the given balerion value
 */
export const setGlobalBalerion = (balerion: Core.Balerion): void => {
  (global as unknown as Global).balerion = balerion;
};
