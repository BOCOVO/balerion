...
import { createLocalBalerionSourceProvider } from '..';

describe('Local Balerion Source Provider', () => {
  describe('Bootstrap', () => {
    test('Should not have a defined Balerion instance if bootstrap has not been called', () => {
      const provider = createLocalBalerionSourceProvider({
        getBalerion: getBalerionFactory({
          db: {
            lifecycles: {
              enable: jest.fn(),
              disable: jest.fn(),
      ...
      expect(provider.balerion).not.toBeDefined();
    });

    test('Should have a defined Balerion instance if bootstrap has been called', async () => {
      const provider = createLocalBalerionSourceProvider({
        getBalerion: getBalerionFactory({
          db: {
            lifecycles: {
      ...
      expect(provider.balerion).toBeDefined();
    });
  });

  describe('Close', () => {
    test('Should destroy the balerion instance if autoDestroy is undefined ', async () => {
    ...
    test('Should destroy the balerion instance if autoDestroy is true ', async () => {
      const destroy = jest.fn();
    ...
    test('Should destroy the balerion instance if autoDestroy is false ', async () => {
      const destroy = jest.fn();
  ...
  describe('Streaming Entities', () => {
    test('Should throw an error if balerion is not defined', async () => {
      const provider = createLocalBalerionSourceProvider({ getBalerion: getBalerionFactory() });

      await expect(() => provider.createEntitiesReadStream()).rejects.toThrowError(
        'Not able to stream entities. Balerion instance not found'
      );
    });
