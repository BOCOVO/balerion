 ...
 */
export const createEntitiesStream = (balerion: Core.Balerion): Readable => {
  const contentTypes: Struct.ContentTypeSchema[] = Object.values(balerion.contentTypes);

  async function* contentTypeStreamGenerator() {
    for (const contentType of contentTypes) {
      const query = shared.entity.createEntityQuery(balerion).call(null, contentType.uid);
