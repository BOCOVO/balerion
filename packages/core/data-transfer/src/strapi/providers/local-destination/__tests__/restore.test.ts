...
describe('Restore ', () => {
  test('Should delete all models and contentTypes', async () => {
    ...
    const { count } = await deleteRecords(balerion);
  ...
  test('Should add core store data', async () => {
    ...
    const result = await restoreConfigs(balerion, config);

    expect(balerion.db.query).toBeCalledTimes(1);
    expect(balerion.db.query).toBeCalledWith('balerion::core-store');
  ...
  test('Should add webhook data', async () => {
    ...
    const result = await restoreConfigs(balerion, config);
    expect(balerion.db.query).toBeCalledTimes(1);
    expect(balerion.db.query).toBeCalledWith('balerion::webhook');
