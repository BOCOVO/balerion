...
import type { Core, UID, Data, Struct } from '@balerion/types';
...
const createEntityQuery = (balerion: Core.Balerion): any => {
  const components = {
  ...
  const query = (uid: UID.Schema) => {
    const create = async <T extends { data: U }, U extends object>(params: T) => {
      ...
      return balerion.db.query(uid).create({ ...params, data: sanitizedData });
    };
    ...
    const createMany = async <T extends { data: U[] }, U extends object>(params: T) => {
      return (
        Promise.resolve(params.data)
          ...
          // Execute a balerion db createMany query with all the entities + their created components
          .then((data) => balerion.db.query(uid).createMany({ ...params, data }))
      );
    };

    const deleteMany = async <T extends object>(params?: T) => {
      const entitiesToDelete = await balerion.db.query(uid).findMany(params ?? {});
      ...
      const deletedEntities = await balerion.db.query(uid).deleteMany(params);
      ...
      for (const [key, attribute] of entries) {
        ...
        if (attribute.type === 'dynamiczone') {
          ...
          for (const componentUID of componentsUID) {
            const component = balerion.getModel(componentUID);
            const subPopulate = getDeepPopulateComponentLikeQuery(component, params);
