...
import { createLocalBalerionDestinationProvider } from '../../providers';
...
export interface PushHandler extends Handler {
  /**
   * Local Balerion Destination Provider used to write data to the current Balerion instance
   */
  provider?: ReturnType<typeof createLocalBalerionDestinationProvider>;
...
export const createPushController = handlerControllerFactory<Partial<PushHandler>>((proto) => ({
  ...
  async init(
    ...
    this.provider = createLocalBalerionDestinationProvider({
      ...params.options,
      autoDestroy: false,
      getBalerion: () => balerion as Core.Balerion,
    });

    this.provider.onWarning = (message) => {
      this.onWarning(message);
      balerion.log.warn(message);
    };
