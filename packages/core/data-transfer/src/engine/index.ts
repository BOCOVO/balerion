...
import type { Struct, Utils } from '@balerion/types';
...
> implements ITransferEngine
{
  ...
  /**
   * Run a version check between two balerion version (source and destination) using the strategy given to the engine during initialization.
   *
   * If there is a mismatch, throws a validation error.
   */
  #assertBalerionVersionIntegrity(sourceVersion?: string, destinationVersion?: string) {
    const strategy = this.options.versionStrategy || DEFAULT_VERSION_STRATEGY;

    const reject = () => {
      throw new TransferEngineValidationError(
        `The source and destination provide are targeting incompatible Balerion versions (using the "${strategy}" strategy). The source (${this.sourceProvider.name}) version is ${sourceVersion} and the destination (${this.destinationProvider.name}) version is ${destinationVersion}`,
        {
          check: 'balerion.version',
          strategy,
  ...
  async integrityCheck() {
    ...
    if (sourceMetadata && destinationMetadata) {
      this.#assertBalerionVersionIntegrity(
        sourceMetadata?.balerion?.version,
        destinationMetadata?.balerion?.version
      );
    }
