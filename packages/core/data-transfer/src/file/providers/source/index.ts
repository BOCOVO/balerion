...
class LocalFileSourceProvider implements ISourceProvider {
    ...
    try {
      ...
      if (this.options?.encryption?.enabled) {
        throw new ProviderInitializationError(
          `Key is incorrect or the file '${filePath}' is not a valid Balerion data file.`
        );
      }
      throw new ProviderInitializationError(`File '${filePath}' is not a valid Balerion data file.`);
    }

    if (!this.#metadata) {
      throw new ProviderInitializationError('Could not load metadata from Balerion data file.');
    }
  }
  ...
  async getSchemas() {
    ...
    if (isEmpty(schemaCollection)) {
      throw new ProviderInitializationError('Could not load schemas from Balerion data file.');
    }
