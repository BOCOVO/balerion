...
const getFolderPath = async (folderId?: number | null) => {
  ...
  const parentFolder = await balerion.db.query(FOLDER_MODEL_UID).findOne({ where: { id: folderId } });

  return parentFolder.path;
};

const deleteByIds = async (ids: number[] = []) => {
  const filesToDelete = await balerion.db
...
const signFileUrls = async (file: File) => {
  ...
  const { provider: providerConfig } = balerion.config.get<Config>('plugin::upload');
