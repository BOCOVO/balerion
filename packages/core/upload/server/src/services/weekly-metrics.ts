...
const getMetricsStoreValue = async (): Promise<MetricStoreValue> => {
  const value = await balerion.store.get({ type: 'plugin', name: 'upload', key: 'metrics' });
  return defaultTo({}, value) as MetricStoreValue;
};
const setMetricsStoreValue = (value: MetricStoreValue) =>
  balerion.store.set({ type: 'plugin', name: 'upload', key: 'metrics', value });
...
export default ({ balerion }: { balerion: Core.Balerion }) => ({
  async computeMetrics() {
    ...
    */
    ...
    // File metrics
    const assetNumber = await balerion.db.query(FILE_MODEL_UID).count();
  ...
  async sendMetrics() {
    ...
    balerion.telemetry.send('didSendUploadPropertiesOnceAWeek', {
