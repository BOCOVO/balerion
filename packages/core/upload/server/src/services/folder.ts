...
import { strings, setCreatorFields } from '@balerion/utils';
...
const setPathIdAndPath = async (folder: Pick<Folder, 'parent'>) => {
  ...
  let parentPath = '/';
  if (folder.parent) {
    const parentFolder = await balerion.db
...
const create = async (
  ...
  const folder = await balerion.db.query(FOLDER_MODEL_UID).create({ data: enrichedFolder });

  balerion.eventHub.emit('media-folder.create', { folder });
...
const deleteByIds = async (ids = []) => {
  const folders = await balerion.db.query(FOLDER_MODEL_UID).findMany({ where: { id: { $in: ids } } });
  if (folders.length === 0) {
    return {
      folders: [],
      totalFolderNumber: 0,
  ...
  // delete files
  const filesToDelete = await balerion.db.query(FILE_MODEL_UID).findMany({
    where: {
  ...
  const { count: totalFolderNumber } = await balerion.db.query(FOLDER_MODEL_UID).deleteMany({
    where: {
      $or: pathsToDelete.flatMap((path) => [
        { path: { $eq: path } },
  ...
  balerion.eventHub.emit('media-folder.delete', { folders });
...
const update = async (
  ...
  if (isUndefined(parent)) {
    const existingFolder = await balerion.db.query(FOLDER_MODEL_UID).findOne({ where: { id } });
    ...
    if (isUndefined(parent)) {
      const folder = await balerion.db
        .query(FOLDER_MODEL_UID)
    ...
    try {
      ...
      const existingFolder = await balerion.db
        .queryBuilder(FOLDER_MODEL_UID)
        .select(['pathId', 'path'])
        .where({ id })
        .transacting(trx.get())
      ...
      // @ts-expect-error - no dynamic types
      const pathColumnName = balerion.db.metadata.get(FOLDER_MODEL_UID).attributes.path.columnName;
      ...
      await balerion.db
        ...
        .update(
          pathColumnName,
          balerion.db.connection.raw('REPLACE(??, ?, ?)', [
            pathColumnName,
            existingFolder.path,
    ...
    balerion.eventHub.emit('media-folder.update', { folder });
    return folder;
  }
};
...
const exists = async (params = {}) => {
  const count = await balerion.db.query(FOLDER_MODEL_UID).count({ where: params });
  return count > 0;
};
...
const getStructure = async () => {
  // @ts-expect-error - no dynamic types
  const { joinTable } = balerion.db.metadata.get(FOLDER_MODEL_UID).attributes.parent;
  const qb = balerion.db.queryBuilder(FOLDER_MODEL_UID);
