...
export default ({ balerion }: { balerion: Core.Balerion }) => {
  ...
  const sendMediaMetrics = (data: Pick<File, 'caption' | 'alternativeText'>) => {
    if (_.has(data, 'caption') && !_.isEmpty(data.caption)) {
      balerion.telemetry.send('didSaveMediaWithCaption');
    }

    if (_.has(data, 'alternativeText') && !_.isEmpty(data.alternativeText)) {
      balerion.telemetry.send('didSaveMediaWithAlternativeText');
    }
  };
  ...
  const createAndAssignTmpWorkingDirectoryToFiles = async (
    ...
    const tmpWorkingDirectory = await fse.mkdtemp(path.join(os.tmpdir(), 'balerion-upload-'));
  ...
  async function emitEvent(event: string, data: Record<string, any>) {
    const modelDef = balerion.getModel(FILE_MODEL_UID);
    ...
    balerion.eventHub.emit(event, { media: sanitizedData });
  }
  ...
  async function enhanceAndValidateFile(
    ...
    const { optimize, isImage, isFaultyImage, isOptimizableImage } = balerion
      .plugin('upload')
  ...
  async function uploadFileAndPersist(fileData: UploadableFile, opts?: CommonOptions) {
    ...
    const config = balerion.config.get<Config>('plugin::upload');
  ...
  async function replace(
    ...
    try {
      ...
      if (dbFile.provider === config.provider) {
        await balerion.plugin('upload').provider.delete(dbFile);

        if (dbFile.formats) {
          await Promise.all(
            Object.keys(dbFile.formats).map((key) => {
              return balerion.plugin('upload').provider.delete(dbFile.formats[key]);
            })
          );
        }
      }
  ...
  async function update(id: ID, values: Partial<File>, opts?: CommonOptions) {
    ...
    const res = await balerion.db.query(FILE_MODEL_UID).update({ where: { id }, data: fileValues });
  ...
  async function add(values: any, opts?: CommonOptions) {
    ...
    const res = await balerion.db.query(FILE_MODEL_UID).create({ data: fileValues });
  ...
  function findOne(id: ID, populate = {}) {
    const query = balerion.get('query-params').transform(FILE_MODEL_UID, {
      populate,
    });

    return balerion.db.query(FILE_MODEL_UID).findOne({
      where: { id },
      ...query,
    });
  }
  ...
  function findMany(query: any = {}): Promise<File[]> {
    return balerion.db
      ...
      .findMany(balerion.get('query-params').transform(FILE_MODEL_UID, query));
  }

  function findPage(query: any = {}) {
    return balerion.db
      .query(FILE_MODEL_UID)
      .findPage(balerion.get('query-params').transform(FILE_MODEL_UID, query));
  }
  ...
  async function remove(file: File) {
    ...
    if (file.provider === config.provider) {
      await balerion.plugin('upload').provider.delete(file);
      ...
      if (file.formats) {
        ...
        await Promise.all(
          keys.map((key) => {
            return balerion.plugin('upload').provider.delete(file.formats![key]);
          })
        );
      }
    }

    const media = await balerion.db.query(FILE_MODEL_UID).findOne({
      where: { id: file.id },
    });

    await emitEvent(MEDIA_DELETE, media);

    return balerion.db.query(FILE_MODEL_UID).delete({ where: { id: file.id } });
  }

  async function getSettings() {
    const res = await balerion.store!({ type: 'plugin', name: 'upload', key: 'settings' }).get({});
  ...
  function setSettings(value: Settings) {
    if (value.responsiveDimensions === true) {
      balerion.telemetry.send('didEnableResponsiveDimensions');
    } else {
      balerion.telemetry.send('didDisableResponsiveDimensions');
    }

    return balerion.store!({ type: 'plugin', name: 'upload', key: 'settings' }).set({ value });
  }

  async function getConfiguration() {
    const res = await balerion.store!({
      type: 'plugin',
      name: 'upload',
  ...
  function setConfiguration(value: ViewConfiguration) {
    return balerion.store!({ type: 'plugin', name: 'upload', key: 'view_configuration' }).set({
      value,
    });
  }
