...
const validateDeleteManyFoldersFilesSchema = yup
  .object()
  .shape({
    fileIds: yup.array().of(yup.balerionID().required()),
    folderIds: yup.array().of(yup.balerionID().required()),
  })
  .noUnknown()
  .required();
...
const validateDuplicatesMoveManyFoldersFilesSchema = yup
  ...
  .test('are-folders-unique', 'some folders already exist', async function areFoldersUnique(value) {
    ...
    const folders = await balerion.db.query(FOLDER_MODEL_UID).findMany({
      select: ['name'],
      where: { id: { $in: folderIds } },
    });

    const existingFolders = await balerion.db.query(FOLDER_MODEL_UID).findMany({
      select: ['name'],
...
const validateMoveFoldersNotInsideThemselvesSchema = yup
  ...
  .test(
    ...
    async function validateMoveFoldersNotInsideThemselves(value) {
      ...
      const destinationFolder = await balerion.db.query(FOLDER_MODEL_UID).findOne({
        select: ['path'],
        where: { id: destinationFolderId },
      });

      const folders: Folder[] = await balerion.db.query(FOLDER_MODEL_UID).findMany({
        select: ['name', 'path'],
        where: { id: { $in: folderIds } },
      });
