...
export default ({ balerion }: { balerion: Core.Balerion }) => {
  const sanitizeOutput = async (data: unknown | unknown[], ctx: Context) => {
    const schema = balerion.getModel(FILE_MODEL_UID);
    const { auth } = ctx.state;

    return balerion.contentAPI.sanitize.output(data, schema, { auth });
  };
  ...
  const validateQuery = async (data: Record<string, unknown>, ctx: Context) => {
    ...
    return balerion.contentAPI.validate.query(data, schema, { auth });
  };
  ...
  const sanitizeQuery = async (data: Record<string, unknown>, ctx: Context) => {
    ...
    return balerion.contentAPI.sanitize.query(data, schema, { auth });
  };
