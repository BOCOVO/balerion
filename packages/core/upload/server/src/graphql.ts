...
export const installGraphqlExtension = ({ balerion }: { balerion: Core.Balerion }) => {
  const { service: getGraphQLService, config: graphQLConfig } = balerion.plugin('graphql');
  const { service: getUploadService } = balerion.plugin('upload');
  ...
  const fileModel = balerion.getModel(FILE_MODEL_UID);
