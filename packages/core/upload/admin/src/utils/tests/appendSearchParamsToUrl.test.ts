...
describe('appendSearchParamsToUrl', () => {
  ...
  describe('relativeURL', () => {
    ...
    beforeAll(() => {
       ...
       * with the backendURL from the balerion window object, here we overwrite it
       * just so it's clear what the expected output is.
       */
      originalBackendURL = window.balerion.backendURL;
      window.balerion.backendURL = 'https://appending-search-params.com';
    });

    afterAll(() => {
      if (originalBackendURL) {
        window.balerion.backendURL = originalBackendURL;
      }
    });
