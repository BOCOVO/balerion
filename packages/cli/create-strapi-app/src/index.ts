...
import { createBalerion } from './create-balerion';
...
const command = new commander.Command('create-balerion-app')
  .version(version)
  .arguments('[directory]')
  ...
  .option('--template <template>', 'Specify a Balerion template')
...
async function run(args: string[]): Promise<void> {
  ...
  const tmpPath = join(os.tmpdir(), `balerion${crypto.randomBytes(6).toString('hex')}`);
  ...
  const scope: Scope = {
    ...
    balerionVersion: version,
    packageJsonBalerion: {
      template: options.template,
    },
    uuid: (process.env.BALERION_UUID_PREFIX || '') + crypto.randomUUID(),
