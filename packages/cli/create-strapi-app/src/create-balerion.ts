...
async function createBalerion(scope: Scope) {
...
async function createApp(scope: Scope) {
  ...
  logger.title('Balerion', `Creating a new application at ${chalk.green(rootPath)}`);
  ...
  logger.title('Balerion', `Your application was created!`);

  logger.log([
    'Available commands in your project:',
    '',
    'Start Balerion in watch mode. (Changes in Balerion project files will trigger a server restart)',
    `${cmd} develop`,
    '',
    'Start Balerion without watch mode.',
    `${cmd} start`,
    '',
    'Build Balerion admin panel.',
    `${cmd} build`,
    '',
    'Deploy Balerion project.',
    `${cmd} deploy`,
    '',
  ]);
  ...
  logger.log(['Display all available commands.', `${cmd} balerion\n`]);
  ...
  if (runApp && installDependencies) {
    logger.title('Run', 'Running your Balerion application');
    ...
    try {
      ...
      logger.fatal('Failed to start your Balerion application');
    }
  }
}
