...
import { cli as cloudCli, services as cloudServices } from '@balerion/cloud-cli';
...
export async function handleCloudLogin(): Promise<void> {
  ...
  const defaultErrorMessage =
    'An error occurred while trying to interact with Balerion Cloud. Use balerion deploy command once the project is generated.';
        ...
        if (e.response.status === 403) {
          const message =
            typeof e.response.data === 'string'
              ? e.response.data
              : 'We are sorry, but we are not able to log you into Balerion Cloud at the moment.';
          logger.warn(message);
          return;
        }
      } catch (e) {
