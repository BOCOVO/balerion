...
export default async (ctx: CLIContext) => {
  ...
  if (environments.length === 0) {
    logger.log(
      `The only available environment is already linked. You can add a new one from your project settings on the Balerion Cloud dashboard.`
    );
    return;
  }
...
async function getEnvironmentsList(
  ...
  try {
    ...
    if (e.response && e.response.status === 404) {
      spinner.succeed();
      ctx.logger.warn(
        `\nThe project associated with this folder does not exist in Balerion Cloud. \nPlease link your local project to an existing Balerion Cloud project using the ${chalk.cyan(
          'link'
        )} command.`
      );
    } else {
      spinner.fail('An error occurred while fetching environments data from Balerion Cloud.');
