...
async function handleError(ctx: CLIContext, error: Error) {
  ...
  if (error instanceof AxiosError) {
    ...
    switch (error.response?.status) {
      ...
      case 503:
        logger.error(
          'Balerion Cloud project creation is currently unavailable. Please try again later.'
        );
        return;
      default:
        if (errorMessage) {
...
async function createProject(ctx: CLIContext, cloudApi: any, projectInput: ProjectInput) {
  ...
  try {
    ...
    spinner.fail('An error occurred while creating the project on Balerion Cloud.');
    throw e;
  }
}
