    ...
    async authCallback({ accessToken }) {
      ...
      return instagram
        ...
        .then(({ body }) => ({
          ...
          email: `${body.username}@balerion.io`, // dummy email as Instagram does not provide user email
        }));
    },
  },
  vk: {
  ...
  reddit: {
    ...
    async authCallback({ accessToken }) {
      ...
      return reddit
        ...
        .then(({ body }) => ({
          username: body.name,
          email: `${body.name}@balerion.io`, // dummy email as Reddit does not provide user email
        }));
    },
  },
    ...
    async authCallback({ accessToken, providers }) {
      ...
      return auth0
        ...
        .then(({ body }) => {
          ...
          const email = body.email || `${username.replace(/\s+/g, '.')}@balerion.io`;
      ...
      return cas
        ...
        .then(({ body }) => {
          ...
          const username = body.attributes
            ? body.attributes.balerionusername || body.id || body.sub
            : body.balerionusername || body.id || body.sub;
          const email = body.attributes
            ? body.attributes.balerionemail || body.attributes.email
            : body.balerionemail || body.email;
          if (!username || !email) {
            balerion.log.warn(
...
module.exports = () => {
  ...
  const baseURL = urljoin(balerion.config.server.url, apiPrefix, 'auth');
