...
module.exports = ({ balerion }) => ({
  ...
  async findRolePermissions(roleID) {
    return balerion.db.query('plugin::users-permissions.role').load({ id: roleID }, 'permissions');
  },
  ...
  async findPublicPermissions() {
    return balerion.db.query('plugin::users-permissions.permission').findMany({
