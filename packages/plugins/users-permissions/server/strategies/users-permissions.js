...
const { ForbiddenError, UnauthorizedError } = require('@balerion/utils').errors;

const { getService } = require('../utils');

const getAdvancedSettings = () => {
  return balerion.store({ type: 'plugin', name: 'users-permissions' }).get({ key: 'advanced' });
};
...
const authenticate = async (ctx) => {
  try {
    ...
    if (token) {
      ...
      const ability = await balerion.contentAPI.permissions.engine.generateAbility(permissions);
    ...
    const ability = await balerion.contentAPI.permissions.engine.generateAbility(publicPermissions);
