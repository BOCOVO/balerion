...
const sanitizeOutput = async (user, ctx) => {
  const schema = balerion.getModel('plugin::users-permissions.user');
  const { auth } = ctx.state;

  return balerion.contentAPI.sanitize.output(user, schema, { auth });
};
...
const validateQuery = async (query, ctx) => {
  ...
  return balerion.contentAPI.validate.query(query, schema, { auth });
};
...
const sanitizeQuery = async (query, ctx) => {
  ...
  return balerion.contentAPI.sanitize.query(query, schema, { auth });
};
...
module.exports = {
  ...
  async create(ctx) {
    ...
    const userWithSameUsername = await balerion.db
    ...
    if (advanced.unique_email) {
      const userWithSameEmail = await balerion.db
        .query('plugin::users-permissions.user')
   ...
   */
  async update(ctx) {
    const advancedConfigs = await balerion
      .store({ type: 'plugin', name: 'users-permissions', key: 'advanced' })
      .get();
