...
const utils = require('@balerion/utils');
...
const sanitizeUser = (user, ctx) => {
  ...
  const userSchema = balerion.getModel('plugin::users-permissions.user');

  return balerion.contentAPI.sanitize.output(user, userSchema, { auth });
};
...
module.exports = ({ balerion }) => ({
  async callback(ctx) {
    ...
    const store = balerion.store({ type: 'plugin', name: 'users-permissions' });
    ...
    if (provider === 'local') {
      ...
      const user = await balerion.db.query('plugin::users-permissions.user').findOne({
        where: {
          provider,
  ...
  async changePassword(ctx) {
    ...
    const validations = balerion.config.get('plugin::users-permissions.validationRules');
  ...
  async connect(ctx, next) {
    ...
    const providers = await balerion
    ...
    if (!balerion.config.server.url.startsWith('http')) {
      balerion.log.warn(
        'You are using a third party provider for login. Make sure to set an absolute url in config/server.js. More info here: https://docs.balerion.io/developer-docs/latest/plugins/users-permissions.html#setting-up-the-server-url'
      );
    }
    ...
    if (customCallback !== undefined) {
      try {
        ...
        const { validate: validateCallback } = balerion
  ...
  async forgotPassword(ctx) {
    ...
    const pluginStore = await balerion.store({ type: 'plugin', name: 'users-permissions' });
    ...
    const emailBody = await getService('users-permissions').template(
      resetPasswordSettings.message,
      {
        ...
        SERVER_URL: balerion.config.get('server.absoluteUrl'),
        ADMIN_URL: balerion.config.get('admin.absoluteUrl'),
        USER: userInfo,
    ...
    await balerion.plugin('email').service('email').send(emailToSend);
  ...
  async register(ctx) {
    ...
    const { register } = balerion.config.get('plugin::users-permissions');
    const alwaysAllowedKeys = ['username', 'password', 'email'];

    // Note that we intentionally do not filter allowedFields to allow a project to explicitly accept private or other Balerion field on registration
    ...
    const conflictingUserCount = await balerion.db.query('plugin::users-permissions.user').count({
