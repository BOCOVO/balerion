...
const errors = require('@balerion/utils');
...
jest.mock('@balerion/utils', () => {
  return {
    ...jest.requireActual('@balerion/utils'),
...
describe('user-permissions auth', () => {
  ...
  describe('register', () => {
    ...
    test.each(registerCases)('$description', async ({ password }) => {
      ...
      const authorization = auth({ balerion: global.balerion });
