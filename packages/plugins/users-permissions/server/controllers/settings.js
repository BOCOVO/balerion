...
const { ValidationError } = require('@balerion/utils').errors;
const { getService } = require('../utils');
const { isValidEmailTemplate } = require('./validation/email-template');

module.exports = {
  async getEmailTemplate(ctx) {
    ctx.send(await balerion.store({ type: 'plugin', name: 'users-permissions', key: 'email' }).get());
  },
  ...
  async getProviders(ctx) {
    ...
    for (const provider in providers) {
      if (provider !== 'email') {
        providers[provider].redirectUri = balerion
