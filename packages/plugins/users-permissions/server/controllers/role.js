...
const { async, errors } = require('@balerion/utils');
...
const sanitizeOutput = async (role) => {
  const { sanitizeLocalizationFields } = balerion.plugin('i18n').service('sanitize');
  const schema = balerion.getModel('plugin::users-permissions.role');
...
module.exports = {
  ...
  async deleteRole(ctx) {
    ...
    // Fetch public role.
    const publicRole = await balerion.db
