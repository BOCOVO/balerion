...
const find = (params: any = {}) =>
  balerion.db.query('plugin::i18n.locale').findMany({ where: params });

const findById = (id: any) => balerion.db.query('plugin::i18n.locale').findOne({ where: { id } });

const findByCode = (code: any) =>
  balerion.db.query('plugin::i18n.locale').findOne({ where: { code } });

const count = (params: any = {}) => balerion.db.query('plugin::i18n.locale').count({ where: params });

const create = async (locale: any) => {
  const result = await balerion.db.query('plugin::i18n.locale').create({ data: locale });
...
const deleteFn = async ({ id }: any) => {
  ...
  if (localeToDelete) {
    ...
    const result = await balerion.db.query('plugin::i18n.locale').delete({ where: { id } });
...
const initDefaultLocale = async () => {
  const existingLocalesNb = await balerion.db.query('plugin::i18n.locale').count();
...
const deleteAllLocalizedEntriesFor = async ({ locale }: any) => {
  ...
  const localizedModels = Object.values(balerion.contentTypes).filter(isLocalizedContentType);

  for (const model of localizedModels) {
    // FIXME: delete many content & their associations
    await balerion.db.query(model.uid).deleteMany({ where: { locale } });
  }
};
