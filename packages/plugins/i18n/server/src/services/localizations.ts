...
const syncNonLocalizedAttributes = async (sourceEntry: any, model: Schema.ContentType) => {
  ...
  const localeEntriesToUpdate = await balerion.db.query(uid).findMany({
    where: {
      documentId,
  ...
  const entryData = await balerion.documents(uid).omitComponentData(nonLocalizedAttributes);

  await async.map(localeEntriesToUpdate, async (entry: any) => {
    const transformedData = await balerion.documents.utils.transformData(
