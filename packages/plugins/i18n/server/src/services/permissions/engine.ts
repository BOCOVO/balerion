...
const willRegisterPermission = (context: any) => {
  ...
  const isSuperAdmin = balerion.service('admin::role').hasSuperAdminRole(user);
  ...
  const ct = balerion.contentTypes[subject];
...
const registerI18nPermissionsHandlers = () => {
  const { engine } = balerion.service('admin::permission');
