...
import { errors, contentTypes as contentTypeUtils } from '@balerion/utils';
    ...
    if (attr.type === 'dynamiczone' && isArray(value)) {
      value.forEach((compo) => {
        if (has('__component', compo)) {
          const model = balerion.components[compo.__component];
          removeIdsMut(model, compo);
        }
      });
    } else if (attr.type === 'component') {
      const model = balerion.components[attr.component];
      if (isArray(value)) {
...
const fillNonLocalizedAttributes = (entry: any, relatedEntry: any, { model }: any) => {
  ...
  const modelDef = balerion.getModel(model);
...
const getNestedPopulateOfNonLocalizedAttributes = (modelUID: any) => {
  const schema = balerion.getModel(modelUID);
