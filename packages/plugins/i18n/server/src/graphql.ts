...
export default ({ balerion }: { balerion: Core.Balerion }) => ({
  register() {
    ...
    const { service: getI18NService } = balerion.plugin('i18n');
    ...
    Object.entries(balerion.contentTypes).forEach(([uid, ct]) => {
...
const getI18nLocaleArgPlugin = ({ nexus, typeRegistry }: any) => {
  ...
  return nexus.plugin({
    ...
    onAddOutputField(config: any) {
      ...
      if (config?.extensions?.balerion?.contentType) {
        contentType = config.extensions.balerion.contentType;
      } else {
