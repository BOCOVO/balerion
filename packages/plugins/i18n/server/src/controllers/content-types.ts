...
const controller = {
  async getNonLocalizedAttributes(ctx) {
    ...
    } = balerion.service('admin::constants');

    const modelDef = balerion.contentType(model);
    ...
    const entity = await balerion.db
      .query(model)
    ...
    const permissions = await balerion.admin.services.permission.findMany({
      where: {
    ...
    const availableLocalesResult = await balerion.plugins['content-manager']
