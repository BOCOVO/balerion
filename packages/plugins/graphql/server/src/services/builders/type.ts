...
export default (context: Context) => {
  const { balerion } = context;
  const getGraphQLService = balerion.plugin('graphql').service;
   ...
   *
   * The attribute is added based on a simple association between a Balerion
   * type and a GraphQL type (the map is defined in `balerionTypeToGraphQLScalar`)
   */
  ...
  const addComponentAttribute = (options: TypeBuildersOptions) => {
    ...
    const targetComponent = balerion.getModel(attribute.component);
  ...
  const addMediaAttribute = (options: TypeBuildersOptions) => {
    ...
    const fileContentType = balerion.contentTypes[fileUID];
  ...
  const addRegularRelationalAttribute = (options: TypeBuildersOptions) => {
    ...
    const targetContentType = balerion.getModel(attribute.target);
      ...
      return objectType({
        ...
        definition(t) {
          ...
          if (
            ...
            balerion.plugin('graphql').config('v4CompatibilityMode', false)
          ) {
            t.nonNull.id('id', {
          ...
          attributesKey
            ...
            .forEach((attributeName) => {
              ...
              // Scalars
              else if (isBalerionScalar(attribute)) {
