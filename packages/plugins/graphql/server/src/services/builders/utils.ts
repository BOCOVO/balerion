...
import { pagination } from '@balerion/utils';
...
export default ({ balerion }: { balerion: Core.Balerion }) => {
  ...
  return {
    ...
    getUniqueScalarAttributes(attributes: Struct.SchemaAttributes) {
      const { isBalerionScalar } = getService('utils').attributes;

      const uniqueAttributes = entries(attributes).filter(
        ([, attribute]) => isBalerionScalar(attribute) && 'unique' in attribute && attribute.unique
      );
    ...
    scalarAttributesToFiltersMap(attributes: Struct.SchemaAttributes) {
      return mapValues((attribute) => {
        ...
        const gqlScalar = mappers.balerionScalarToGraphQLScalar(attribute.type);
    ...
    transformArgs(
      ...
      if (args.filters) {
        Object.assign(newArgs, {
          filters: mappers.graphQLFiltersToBalerionQuery(filters, contentType),
        });
      }
