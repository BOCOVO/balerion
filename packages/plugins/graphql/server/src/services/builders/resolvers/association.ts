...
export default ({ balerion }: Context) => {
  ...
  return {
    buildAssociationResolver({
      ...
      const targetContentType = balerion.getModel(targetUID);
      ...
      return async (parent: any, args: any = {}, context: any = {}) => {
        ...
        await balerion.contentAPI.validate.query(transformedArgs, targetContentType, {
          auth,
        });

        const sanitizedQuery = await balerion.contentAPI.sanitize.query(
          transformedArgs,
          targetContentType,
          {
            auth,
        ...
        const dbQuery = balerion.get('query-params').transform(targetUID, sanitizedQuery);

        const data = await balerion.db?.query(contentTypeUID).load(parent, attributeName, dbQuery);
        ...
        if (isMorphAttribute) {
          ...
          const sanitizeData = (dataToSanitize: any) => {
            return balerion.contentAPI.sanitize.output(dataToSanitize, contentType, { auth });
          };
