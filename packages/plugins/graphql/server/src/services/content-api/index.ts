...
export default ({ balerion }: { balerion: Core.Balerion }) => {
  ...
  const { service: getGraphQLService } = balerion.plugin('graphql');
  ...
  const buildSchema = () => {
    ...
    registerScalars({ registry, balerion });
    registerInternals({ registry, balerion });
    ...
    const currentEnv = balerion.config.get('environment');
    ...
    const wrappedNexusSchema = wrapResolvers({ schema: nexusSchema, balerion, extension });
  ...
  const shadowCRUD = () => {
    ...
    // Get every content type & component defined in Balerion
    const contentTypes = [
      ...Object.values(balerion.components),
      ...Object.values(balerion.contentTypes),
    ];
  ...
  const registerAPITypes = (contentTypes: Struct.Schema[]) => {
    for (const contentType of contentTypes) {
      ...
      const registerOptions = { registry, balerion, builders };
  ...
  const registerMorphTypes = (contentTypes: Struct.Schema[]) => {
    ...
    for (const contentType of contentTypes) {
      registerPolymorphicContentType(contentType, { registry, balerion });
    }
  };
