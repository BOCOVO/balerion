...
const formatToCode = (name: string) => `BALERION_${toUpper(snakeCase(name))}`;
 ...
 *
 * Intercepts specific Balerion error types to send custom error response codes in the GraphQL response
 */
export function formatGraphqlError(formattedError: GraphQLFormattedError, error: unknown) {
  ...
  // else if originalError doesn't appear to be from Balerion or GraphQL..

  // Log the error
  balerion.log.error(originalError);
