...
const createService = ({ balerion }: { balerion: Core.Balerion }) => {
  const config = balerion.config.get('plugin::documentation') as PluginConfig;
  ...
  return {
    ...
    getFullDocumentationPath() {
      return path.join(balerion.dirs.app.extensions, 'documentation', 'documentation');
    },
    ...
    async getDocumentationAccess() {
      const { restrictedAccess } = (await balerion.store!({
        environment: '',
        type: 'plugin',
        name: 'documentation',
        key: 'config',
    ...
    getApiDocumentationPath(api: { name: string; getter: string }) {
      if (api.getter === 'plugin') {
        return path.join(balerion.dirs.app.extensions, api.name, 'documentation');
      }

      return path.join(balerion.dirs.app.api, api.name, 'documentation');
    },
    ...
    getPluginAndApiInfo() {
      const pluginsToDocument = pluginsThatNeedDocumentation.map((plugin) => {
        return {
          name: plugin,
          getter: 'plugin',
          ctNames: Object.keys(balerion.plugin(plugin).contentTypes),
        };
      });

      const apisToDocument = Object.keys(balerion.apis).map((api) => {
        return {
          name: api,
          getter: 'api',
          ctNames: Object.keys(balerion.api(api).contentTypes),
        };
      });
    ...
    async generateFullDoc(versionOpt?: string) {
      ...
      const generatedDocumentation = await produce(config, async (draft) => {
        if (draft.servers?.length === 0) {
          ...
          const apiPath = balerion.config.get('api.rest.prefix');
          draft.servers = [
            {
        ...
        draft['x-balerion-config'].plugins = pluginsThatNeedDocumentation;

        // Delete the mutateDocumentation key from the config so it doesn't end up in the spec
        delete draft['x-balerion-config'].mutateDocumentation;
      ...
      const userMutatesDocumentation = config['x-balerion-config'].mutateDocumentation;
