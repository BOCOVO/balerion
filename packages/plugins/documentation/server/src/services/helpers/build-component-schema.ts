...
const getAllSchemasForContentType = ({ routeInfo, attributes, uniqueName }: ApiInfo) => {
  ...
  const didAddBalerionComponentsToSchemas = (schemaName: string, schema: OpenAPIV3.SchemaObject) => {
    if (!Object.keys(schema) || !Object.keys(schema.properties!)) return false;

    // Add the Balerion components to the schema
    balerionComponentSchemas = {
      ...balerionComponentSchemas,
      [schemaName]: schema,
    };
  ...
  Object.assign(schemas, {
    [`${typeName}`]: {
      ...
      properties: {
        ...
        ...cleanSchemaAttributes(attributes, { didAddBalerionComponentsToSchemas }),
      },
    },
  ...
  return { ...schemas, ...balerionComponentSchemas };
};
