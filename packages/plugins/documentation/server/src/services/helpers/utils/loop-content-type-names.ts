...
const loopContentTypeNames = (api: Api, callback: (info: ApiInfo) => any) => {
  ...
  for (const contentTypeName of api.ctNames) {
    ...
    const { attributes, info: contentTypeInfo, kind } = balerion.contentType(uid as any);

    // Get the routes for the current api
    const routeInfo =
      api.getter === 'plugin'
        ? // @ts-expect-error – TODO: fix this
          balerion.plugin(api.name).routes['content-api']
        : balerion.api(api.name).routes[contentTypeName];
