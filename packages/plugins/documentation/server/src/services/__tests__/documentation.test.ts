...
import { apis, plugins, components, contentTypes } from '../__mocks__/mock-balerion-data';
import documentation from '../documentation';
import override from '../override';
import { defaultConfig } from '../../config/default-plugin-config';

const mockBalerionInstance = {
  dirs: {
    app: {
      api: './',
      extensions: './',
...
describe('Documentation plugin | Documentation service', () => {
  beforeAll(() => {
    global.balerion = mockBalerionInstance;
    global.balerion.contentType = jest.fn((uid) => {
      // Only deal with mocked data, return empty attributes for unmocked relations
      if (!global.balerion.contentTypes[uid]) return { attributes: {} };

      return global.balerion.contentTypes[uid];
    }) as any;

    global.balerion.plugins.documentation = {
      service: jest.fn((name) => {
        const mockServices = {
          override: override({ balerion: global.balerion }),
        } as any;
  ...
  afterAll(() => {
    // Teardown the mocked balerion instance
    global.balerion = {} as any;
  });

  afterEach(() => {
    // Reset the mocked balerion config
    global.balerion.config.get = () => defaultConfig as any;
  });

  it('generates a valid openapi schema', async () => {
    const docService = documentation({ balerion: global.balerion });
  ...
  describe('Determines the plugins that need documentation', () => {
    it('generates documentation for the default plugins if the user provided nothing in the config', async () => {
      ...
      expect(mockFinalDoc['x-balerion-config'].plugins).toEqual(['upload', 'users-permissions']);
    });

    it("generates documentation only for plugins in the user's config", async () => {
      global.balerion.config.get = () =>
        ({
          ...defaultConfig,
          'x-balerion-config': { ...defaultConfig['x-balerion-config'], plugins: ['upload'] },
        }) as any;
      ...
      const mockFinalDoc = lastMockCall[1];
      expect(mockFinalDoc['x-balerion-config'].plugins).toEqual(['upload']);
    });
    ...
    it('does not generate documentation for any plugins', async () => {
      global.balerion.config.get = () =>
        ({
          ...defaultConfig,
          'x-balerion-config': { ...defaultConfig['x-balerion-config'], plugins: [] },
        }) as any;
      ...
      const mockFinalDoc = lastMockCall[1];
      expect(mockFinalDoc['x-balerion-config'].plugins).toEqual([]);
    });
  });
  ...
  describe('Handles user config and overrides', () => {
    it('replaces default config with the user config', async () => {
      ...
      global.balerion.config.get = () => ({ ...userConfig }) as any;
      const docService = documentation({ balerion: global.balerion });
      await docService.generateFullDoc();
      const lastMockCall = jest.mocked(fse.writeJson).mock.calls[
        jest.mocked(fse.writeJson).mock.calls.length - 1
      ...
      expect(mockFinalDoc['x-balerion-config']).toEqual(userConfig['x-balerion-config']);
    ...
    it("does not apply an override if the plugin providing the override isn't specified in the x-balerion-config.plugins", async () => {
      global.balerion.config.get = () =>
        ({
          ...defaultConfig,
          'x-balerion-config': { ...defaultConfig['x-balerion-config'], plugins: [] },
      ...
      const docService = documentation({ balerion: global.balerion });
      const overrideService = override({ balerion: global.balerion });
    ...
    it("applies a user's mutateDocumentation function", async () => {
      global.balerion.config.get = () =>
        ({
          ...defaultConfig,
          'x-balerion-config': {
            ...defaultConfig['x-balerion-config'],
