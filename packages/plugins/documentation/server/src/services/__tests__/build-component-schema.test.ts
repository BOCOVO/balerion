...
describe('Documentation plugin | Build component schema', () => {
  beforeEach(() => {
    // Reset the mocked balerion instance
    global.balerion = _.cloneDeep(balerion);
  });
  ...
  it('builds the ResponseList schema', () => {
    (global.balerion.plugins['users-permissions'].routes as any)['content-api'].routes = [
      { method: 'GET', path: '/test', handler: 'test.find' },
    ];
    global.balerion.apis.restaurant.routes.restaurant.routes = [
      { method: 'GET', path: '/test', handler: 'test.find' },
    ] as any;
