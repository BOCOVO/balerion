...
export default {
    ...
    try {
      ...
      try {
        ...
        const filledLayout = _.template(layout)({
          backendUrl: balerion.config.server.url,
  ...
  async loginView(ctx: Koa.Context, next: Koa.Next) {
    ...
    try {
      ...
      const filledLayout = _.template(layout.toString())({
        actionUrl: `${balerion.config.server.url}/documentation/login`,
      });
      ...
      try {
        ...
        try {
          const staticFolder = path.resolve(balerion.dirs.app.extensions, 'documentation', 'public');
          return koaStatic(staticFolder)(ctx, next);
        } catch (e) {
          balerion.log.error(e);
        }
  ...
  async login(ctx: Koa.Context) {
    ...
    const { password: hash } = (await balerion
    ...
    ctx.redirect(`${balerion.config.server.url}/documentation${querystring}`);
  },
  ...
  async regenerateDoc(ctx: Koa.Context) {
    ...
    try {
      ...
      balerion.reload.isWatching = true;
    }
  },
  ...
  async updateSettings(ctx: Koa.Context) {
    const pluginStore = balerion.store({ type: 'plugin', name: 'documentation' });
