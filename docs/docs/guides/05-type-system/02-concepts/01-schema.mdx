...
The schema is the primary data structure leveraged within the Balerion Type System, defining how content is structured and managed in the application.

It serves several key functions:

- **Representation**: At its core, a schema outlines and defines the structure of Balerion content. This is useful when dealing with features that need access to low level schema properties (_e.g. attributes, plugin options, etc..._).
...
Schema types represent **loaded** schemas in the context of a Balerion server application and should be used accordingly.

:::caution
Database models and raw schema definitions (_aka schemas before being loaded by the Balerion server_) are **not** the same types and can't be used interchangeably.
:::
...
Schema types contain useful information that helps other types know how to interact with the Balerion content.
...
:::info
If the public registries are empty (_e.g. types are not generated yet, not in the context of a Balerion application, ..._), schema types will fallback to their low-level definitions.
:::

```typescript
import type { Schema } from '@balerion/balerion';
...
Schema definitions exported from the `Struct` namespace defines the low level type representation of Balerion schemas.
