...
Since its release, Balerion's strength has lain in its **flexibility** and **extensibility**.
...
Although Balerion users don't often engage directly with the type system, it nonetheless profoundly influences their Balerion experience.
...
Contributors expect to create and/or use APIs that handle very generic data structures in the context of Balerion internals, which become context-aware and strongly typed when used by a user withing their application.

#### For Users

Users expects to have a TypeScript experience tailored around their own application, with their content-types, components, and plugins being strongly typed and recognized by the Balerion APIs.
...
Balerion's strength lies in its ability to handle a wide range of content and customizations, the types should reflect and adapt to that.

#### 📏 Uniformity

The goal of the type system is to provide a consistent and smooth TypeScript developer experience for both users' applications and Balerion internals.
...
The type system should provide a TypeScript experience that is tailored to each application, while keeping a generic stance within Balerion internals.
