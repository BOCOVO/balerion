---
title: Transactions
description: Conceptual guide to transactions in Balerion
tags:
  - database
  - experimental
---
...
Transactions are handled by passing a handler function into `balerion.db.transaction`:

```js
await balerion.db.transaction(async ({ trx, rollback, commit, onCommit, onRollback }) => {
  // It will implicitly use the transaction
  await balerion.db.create();
  await balerion.db.create();
});
```
...
:::note
Every `balerion.db.query` operation performed in a transaction block will implicitly use the transaction.
:::
...
```js
await balerion.db.transaction(async ({ onCommit, onRollback }) => {
  // It will implicitly use the transaction
  await balerion.db.create();
  await balerion.db.create();
...
```js
await balerion.db.transaction(async ({ trx, rollback, commit }) => {
