...
The information which stage is currently assigned to an entity is send as part of the content-type response payload for each entity in the attribute `balerion_stage`.
Please refer to the [type definitions](/settings/review-workflows) for more information.

```ts
{
  // ... entity attributes
  balerion_stage?: Stage | null
}
```
...
Information about which stage is currently assigned to an entity is sent as part of the entity response payload in the attribute `balerion_stage`.
Please refer to the [type definitions](/settings/review-workflows) for more information.
