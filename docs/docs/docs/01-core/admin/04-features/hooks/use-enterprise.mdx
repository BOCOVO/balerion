...
A hook that returns either community or enterprise-edition data-structures based on the global `window.balerion.isEE` flag.
...
By default the hook returns `null` if `window.balerion.isEE` is true and the enterprise data structure is not yet loaded. Customizing
