...
const edition = process.env.BALERION_DISABLE_EE === 'true' ? 'CE' : 'EE';
...
describeOnCondition(edition === 'EE')('Review workflows', () => {
  ...
  beforeAll(async () => {
    ...
    hasRW = balerion.ee.features.isEnabled('review-workflows');

    requests.admin = await createAuthRequest({ balerion });
    // @ts-expect-error - We don't have the type for this
    requests.public = createRequest({ balerion }).setToken(await getFullAccessToken());

    defaultStage = await balerion.db.query(STAGE_MODEL_UID).create({
      data: { name: 'Stage' },
    });
    secondStage = await balerion.db.query(STAGE_MODEL_UID).create({
      data: { name: 'Stage 2' },
    });
    testWorkflow = await balerion.db.query(WORKFLOW_MODEL_UID).create({
      data: {
        contentTypes: [],
        name: 'workflow',
  ...
  beforeEach(async () => {
    testWorkflow = await balerion.db.query(WORKFLOW_MODEL_UID).update({
    ...
    defaultStage = await balerion.db.query(STAGE_MODEL_UID).update({
      where: { id: defaultStage.id },
      data: { name: 'Stage' },
    });
    secondStage = await balerion.db.query(STAGE_MODEL_UID).update({
  ...
  describe('Assignee updates in Content Manager', () => {
    describe('Can update assignee on an entity if review workflow is enabled', () => {
      ...
      test('Correctly sanitize private fields of assignees in the content API', async () => {
        const assigneeAttribute = 'balerion_assignee';
  ...
  describe('Stage updates in Content Manager', () => {
    ...
    describe('Update a stage on an entity', () => {
      describe('Review Workflow is enabled', () => {
        ...
        test('Should return entity stage information to the content API', async () => {
          const stageAttribute = 'balerion_stage';
    ...
    describe('Listing available stages for transition', () => {
      ...
      beforeAll(async () => {
        ...
        // @ts-expect-error - We don't have the type for this
        restrictedRequest = await createAuthRequest({ balerion, userInfo: restrictedUserInfo });
      });
