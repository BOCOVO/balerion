...
describeOnCondition(edition === 'EE')('Review workflows', () => {
  ...
  beforeAll(async () => {
    ...
    stageA = await balerion.db.query(STAGE_MODEL_UID).create({
      data: { name: 'Stage A' },
    });
    stageB = await balerion.db.query(STAGE_MODEL_UID).create({
      data: { name: 'Stage B' },
    });
    workflow = await balerion.db.query(WORKFLOW_MODEL_UID).create({
      data: {
        contentTypes: [],
        name: 'workflow',
  ...
  test(`Updating entity stage should trigger ${WORKFLOW_UPDATE_STAGE}`, async () => {
    ...
    balerion.eventHub.on(WORKFLOW_UPDATE_STAGE, (payload) => {
