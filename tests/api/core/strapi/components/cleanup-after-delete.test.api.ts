...
describe('Component Deletion and Cleanup Test', () => {
  ...
  test('Create and delete component, then verify cleanup', async () => {
    ...
    const dbResultBeforeDeletion = await balerion.db.connection
      .select('*')
    ...
    const dbResult = await balerion.db.connection.raw(`SELECT * FROM ${contentType.pluralName}_cmps`);

    // Ensure table for the deleted component no longer exists
    const tempComponentTableExists = await balerion.db.connection.schema.hasTable(
    ...
    const keepComponentTableExists = await balerion.db.connection.schema.hasTable(
