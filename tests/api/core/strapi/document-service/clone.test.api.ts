...
describe('Document Service', () => {
  ...
  describe('clone', () => {
    testInTransaction('clone a document locale', async () => {
      ...
      const result = await balerion.documents(ARTICLE_UID).clone({
    ...
    testInTransaction('clone a document with components', async () => {
      ...
      // update article
      const clonedArticle = await balerion.documents(ARTICLE_UID).clone({
        documentId,
        locale: 'en',
        data: {
    ...
    testInTransaction('clone non existing document', async () => {
      const resultPromise = await balerion.documents(ARTICLE_UID).clone({
        documentId: '1234',
        data: {
          title: 'Cloned Document',
        },
