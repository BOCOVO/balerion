...
describe('Document Service relations', () => {
  ...
  describe.skip('Update', () => {
    testInTransaction('Can update a document with relations', async () => {
      const article = await balerion.documents(ARTICLE_UID).update({
        documentId: 'Article1',
        locale: 'en',
        data: {
          title: 'Article with author',
  ...
  describe('Publish', () => {
    testInTransaction(
      ...
      async () => {
        ...
        const publishedArticles = await balerion.documents(ARTICLE_UID).publish({
    ...
    testInTransaction(
      ...
      async () => {
        ...
        await balerion.documents(CATEGORY_UID).publish({ documentId: 'Cat1', locale: 'en' });
  ...
  describe('Discard', () => {
    testInTransaction(
      ...
      async () => {
        ...
        const draftArticle = await balerion.documents(ARTICLE_UID).create({
          data: { title: 'Article with author', categories: ['Cat1'] },
        });
        ...
        // Publish documents
        await balerion.documents(CATEGORY_UID).publish({ documentId: 'Cat1' });
        await balerion.documents(ARTICLE_UID).publish({ documentId: id });
        ...
        // Discard the draft
        const newDraftArticles = await balerion
