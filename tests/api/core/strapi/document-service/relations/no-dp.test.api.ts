...
describe('Relations interactions with disabled DP content types', () => {
  beforeAll(async () => {
    ...
    // PRODUCTS
    await balerion.db.query(PRODUCT_UID).createMany({
      data: [
    ...
    await balerion.db.query(SHOP_UID).createMany({
      data: [
    ...
    productDocuments = balerion.documents(PRODUCT_UID);
    tagDocuments = balerion.documents(TAG_UID);
