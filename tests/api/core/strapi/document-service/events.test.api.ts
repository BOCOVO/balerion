...
describe('Document Service Events', () => {
  ...
  afterEach(async () => {
    balerion.eventHub.removeAllListeners();
  });
  ...
  describe('Creates', () => {
    it('Creating a document triggers entry.create', async () => {
      ...
      balerion.eventHub.on('entry.create', async (payload) => {
        expect(payload).toMatchObject(eventPayload);
      });

      await balerion.documents(ARTICLE_UID).create({ data: articleData });
    });
  });
  ...
  describe('Update', () => {
    it('Updating a document locale triggers entry.update', async () => {
      ...
      // Create new article
      const article = await balerion.documents(ARTICLE_UID).create({ data: articleData });

      balerion.eventHub.on('entry.update', async (payload) => {
      ...
      await balerion.documents(ARTICLE_UID).update({
    ...
    it('Updating and publishing triggers both entry.update and entry.publish', async () => {
      ...
      balerion.eventHub.on('entry.publish', async (payload) => {
        expect(payload).toMatchObject({
  ...
  describe('Delete', () => {
    it('Deleting a document triggers entry.delete', async () => {
      ...
      balerion.eventHub.on('entry.delete', async (payload) => {
      ...
      await balerion.documents(ARTICLE_UID).delete({ documentId: article.documentId });
    });
  });
  ...
  describe('Unpublish', () => {
    it('Unpublishing a document triggers entry.unpublish', async () => {
      ...
      balerion.eventHub.on('entry.unpublish', async (payload) => {
      ...
      await balerion.documents(ARTICLE_UID).unpublish({ documentId: article.documentId });
    });
  });
  ...
  describe('Discard draft', () => {
    it('Discarding a draft triggers entry.draft-discard', async () => {
      ...
      balerion.eventHub.on('entry.draft-discard', async (payload) => {
        expect(payload).toMatchObject({
          ...eventPayload,
          entry: {
            publishedAt: null,
      ...
      await balerion.documents(ARTICLE_UID).discardDraft({ documentId: article.documentId });
    });
  });
});
