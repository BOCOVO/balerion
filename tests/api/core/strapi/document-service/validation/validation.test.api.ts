  ...
  describe.each(methods)('%s method', (methodName) => {
    describe('sort', () => {
      it('should not throw on existing attribute name', async () => {
        await balerion.documents(ARTICLE_UID)[methodName]({ sort: 'title' });
      });

      it('should not throw on private attribute', async () => {
        await balerion.documents(ARTICLE_UID)[methodName]({ sort: 'private' });
      });

      it('should not throw on password attribute', async () => {
        await balerion.documents(ARTICLE_UID)[methodName]({ sort: 'password' });
      });

      it('should not throw on existing nested (object) key', async () => {
        await balerion.documents(ARTICLE_UID)[methodName]({
      ...
      it('should throw ValidationError on invalid key', async () => {
        await expect(
          balerion.documents(ARTICLE_UID)[methodName]({ sort: 'fakekey' })
    ...
    describe('filters', () => {
      ...
      it('should throw ValidationError on invalid key', async () => {
        await expect(
          balerion.documents(ARTICLE_UID)[methodName]({
            filters: {
              fakekey: 'Hello World',
            },
          })
    ...
    describe('fields', () => {
      ...
      it('should not throw on valid comma separated keys', async () => {
        await balerion.documents(ARTICLE_UID)[methodName]({ fields: 'title,password,private' });
      });

      it('should throw on invalid comma separated keys', async () => {
        await expect(
          balerion.documents(ARTICLE_UID)[methodName]({ fields: 'title,invalid' })
        ).rejects.toThrow(errors.ValidationError);
      });
    });
