...
describe('Document Service', () => {
  ...
  describe('Scalar unique fields', () => {
    it('can create a draft document with a duplicated unique field value', async () => {
      await balerion.documents(CATEGORY_UID).create({
        data: { name: testName },
      });
    });
    ...
    it('can update a draft document to have a duplicated unique field value', async () => {
      ...
      const category: Category = await balerion.documents(CATEGORY_UID).create({
        data: { name: uniqueName },
      });
      createdCategory = category;

      await balerion.documents(CATEGORY_UID).update({
    ...
    it('cannot publish a document to have a duplicated unique field value in the same publication state', async () => {
      ...
      const category = await balerion.documents(CATEGORY_UID).create({ data: { name } });

      // Publish that category
      const publishRes = balerion
      ...
      const newCategory = await balerion.documents(CATEGORY_UID).create({ data: { name } });
      expect(
        balerion.documents(CATEGORY_UID).publish({ documentId: newCategory.documentId })
      ).rejects.toThrow();
    });
    ...
    it('can save and publish multiple entries with an empty string in a unique field', async () => {
      ...
      const category2 = await balerion.documents(CATEGORY_UID).create({ data: { name: '' } });
      expect(category2).toBeDefined();

      // Publish categories, no error should be thrown
      await balerion.documents(CATEGORY_UID).publish({ documentId: category.documentId });
      await balerion.documents(CATEGORY_UID).publish({ documentId: category2.documentId });
    });
  });
      ...
      for (const { description, createData } of testCases) {
        it(`cannot create multiple entities with the same unique ${field} value in the same ${description}, locale and publication state`, async () => {
          ...
          if (isRepeatable) {
            ...
            const createdArticle = await balerion.documents(ARTICLE_UID).create({
              data: createData(field, value, true),
            });

            await expect(
              balerion.documents(ARTICLE_UID).publish({
          ...
          const article = await balerion.documents(ARTICLE_UID).create({
            data: createData(field, value),
          });
          await balerion.documents(ARTICLE_UID).publish({ documentId: article.documentId });
          ...
          const articleDifferentLocale = await balerion.documents(ARTICLE_UID).create({
            data: createData(field, value),
            locale: otherLocale,
          });
          ...
          await expect(
            balerion.documents(ARTICLE_UID).publish({ documentId: createdArticle.documentId })
          ...
          const secondArticle = await balerion.documents(ARTICLE_UID).create({
