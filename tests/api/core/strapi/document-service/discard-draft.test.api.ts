...
describe('Document Service', () => {
  ...
  describe('Discard Draft', () => {
    ...
    beforeAll(async () => {
      ...
      await balerion.documents(ARTICLE_UID).publish({ documentId, locale: '*' });

      // Update drafts
      await Promise.all(
        ['es', 'nl', 'en'].map((locale) =>
          balerion.documents(ARTICLE_UID).update({
            documentId,
            locale,
            data: {
    ...
    testInTransaction('Discard all locale drafts of a document', async () => {
      // Discard drafts
      const result = await balerion.documents(ARTICLE_UID).discardDraft({ documentId, locale: '*' });
    ...
    testInTransaction('Discard a single locale of a document', async () => {
      ...
      const result = await balerion.documents(ARTICLE_UID).discardDraft({ documentId, locale: 'en' });
