...
const createEntry = async (data) => {
  return balerion.db.query('api::nonvisible.nonvisible').create({
    data,
  });
};
...
describe.skip('Test non visible fields', () => {
  beforeAll(async () => {
    ...
    user2 = await utils.createUser({ ...userInfo, email: 'test2@balerion.io' });

    rq1 = await createAuthRequest({ balerion, userInfo: user1 });
    rq2 = await createAuthRequest({ balerion, userInfo: user2 });
