  ...
  describe.each([{ populateRelations: false }, { populateRelations: true }])(
    ...
    ({ populateRelations }) => {
      beforeAll(async () => {
        ...
        balerion.config.set('server.webhooks.populateRelations', populateRelations);

        data.collectors = await builder.sanitizedFixturesFor(collector.singularName, balerion);
        data.stamps = await builder.sanitizedFixturesFor(stamp.singularName, balerion);
      });
