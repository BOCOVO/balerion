...
describeOnCondition(edition === 'EE')('History API', () => {
    ...
    const res = await rq({
      ...
      formData: {
        files: [
          ...
          fs.createReadStream(path.join(__dirname, 'balerion.jpg')),
        ],
      },
    });
  ...
  const createUserAndReq = async (
    ...
    const utils = createUtils(balerion);
    ...
    const user = await utils.createUser({
      firstname: userName,
      lastname: 'User',
      email: `${userName}.user@balerion.io`,
      roles: [role.id],
    });

    const rq = await createAuthRequest({ balerion, userInfo: user });
  ...
  beforeAll(async () => {
    ...
    const localeService = balerion.plugin('i18n').service('locales');
  ...
  afterAll(async () => {
    ...
    await balerion.db.query('plugin::i18n.locale').deleteMany({ where: { code: { $ne: 'en' } } });
  ...
  describe('Find many history versions', () => {
    ...
    test('Creates a history version when cloning an entry', async () => {
      ...
      const { documentId: currentDocumentId, ...currentDocumentData } = await balerion
  ...
  describe('Restore a history version', () => {
    ...
    test('Restores a history version in the default locale', async () => {
      ...
      const restoredDocument = await balerion
        .documents(collectionTypeUid)
    ...
    test('Restores a history version with missing relations', async () => {
      ...
      // Delete a relation
      await balerion.documents(relationUid).delete({ documentId: relations[0].data.documentId });
      ...
      const restoredDocument = await balerion.documents(collectionTypeUid).findOne({
        documentId: collectionTypeDocumentId,
