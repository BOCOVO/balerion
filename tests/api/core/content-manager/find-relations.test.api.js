...
describe('Find Relations', () => {
  ...
  beforeAll(async () => {
    ...
    const [skate, chair, candle, table, porte, fenetre] = await Promise.all([
      balerion.documents(productUid).create({ data: { name: 'Skate' } }),
      balerion.documents(productUid).create({ data: { name: 'Chair' } }),
      balerion.documents(productUid).create({ data: { name: 'Candle' } }),
      balerion.documents(productUid).create({ data: { name: 'Table' } }),
      // We create products in French in order to test that we can cant find
      // available relations in a different locale
      balerion.documents(productUid).create({ data: { name: 'Porte' }, locale: extraLocale }),
      balerion.documents(productUid).create({ data: { name: 'Fenetre' }, locale: extraLocale }),
    ]);
    ...
    const [publishedSkate, publishedChair] = await Promise.all([
      balerion.documents(productUid).publish({ documentId: productMapping.skate.documentId }),
      balerion.documents(productUid).publish({ documentId: productMapping.chair.documentId }),
    ]);
    data[productUid].published.push(publishedSkate.entries[0], publishedChair.entries[0]);

    const stan = await balerion.documents(employeeUid).create({ data: { name: 'Stan' } });
    ...
    const [draftShop, draftEmptyShop] = await Promise.all([
      balerion.documents(shopUid).create({
        data: {
          name: 'Cazotte Shop',
    ...
    const [publishedShop, publishedEmptyShop] = await Promise.all([
      balerion.documents(shopUid).publish({
        documentId: draftShop.documentId,
        populate: Object.keys(allRelations),
      }),
      balerion.documents(shopUid).publish({ documentId: draftEmptyShop.documentId }),
    ]);
