  ...
  describe('Read', () => {
    ...
    beforeAll(async () => {
      ...
      u1Req = await createAuthRequest({
        balerion,
        userInfo: { email: 'reader1@balerion.io', password: 'Reader1' },
      });

      u2Req = await createAuthRequest({
        balerion,
        userInfo: { email: 'reader2@balerion.io', password: 'Reader2' },
      });
    });
