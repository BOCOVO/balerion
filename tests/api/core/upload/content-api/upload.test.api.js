...
describe('Upload plugin', () => {
  beforeAll(async () => {
    ...
    rq = createContentAPIRequest({ balerion });
  });
  ...
  describe('Read', () => {
    ...
    test('Get one file', async () => {
      ...
      const dogEntity = await balerion.db.query('api::dog.dog').create({
        data: {
      ...
      await balerion.db.query('api::dog.dog').delete({ where: { id: dogEntity.id } });
      await balerion.db
  ...
  describe('Filtering data based on media attributes', () => {
    ...
    beforeAll(async () => {
      await Promise.all(
        data.dogs.map((dog) => {
          return balerion.entityService.delete('api::dog.dog', dog.data.id);
        })
      );
