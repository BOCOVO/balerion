...
describe('Homepage API', () => {
  ...
  it('finds the most recently updated documents', async () => {
    ...
    const globalDoc = await balerion.documents(globalUid).create({
      data: {
        siteName: 'a cool site name',
      },
    });
    await balerion.documents(globalUid).publish({
    ...
    for (let i = 0; i < 9; i++) {
      if (i % 3 === 0) {
        // When index is 0, 3, 6
        await balerion.documents(articleUid).create({
          data: {
            title: `Article ${i}`,
        ...
        // When index is 1, 4, 7
        await balerion.documents(globalUid).update({
          documentId: globalDoc.documentId,
          status: 'draft',
          data: {
            siteName: `global-${i}`,
        ...
        // When index is 2, 5, 8
        await balerion.documents(authorUid).create({
          data: {
            name: `author-${i}`,
          },
        });
  ...
  it('finds the most recently published documents', async () => {
    ...
    const article = await balerion.documents(articleUid).create({
      data: {
        title: 'The Paperback Writer',
      },
    });
    const tag = await balerion.documents(tagUid).create({
      data: {
        slug: 'Tag 1',
      },
    });
    // Create non draft and publish document
    const author = await balerion.documents(authorUid).create({
      data: {
        name: 'Paul McCartney',
      },
    });
    ...
    // Publish the article
    await balerion.documents(articleUid).publish({
      documentId: article.documentId,
    });
    // Update published document to create a 'modified' status
    await balerion.documents(articleUid).update({
      documentId: article.documentId,
      data: {
        title: 'Paperback Writer',
      },
    });
    await balerion.documents(tagUid).publish({
      documentId: tag.documentId,
    });
    // Update the author (won't be included in the response)
    await balerion.documents(authorUid).update({
