...
describe('Admin API Token v2 CRUD (api)', () => {
  ...
  const deleteAllTokens = async () => {
    const tokens = await balerion.service('admin::api-token').list();
    const promises = [];
    tokens.forEach(({ id }) => {
      promises.push(balerion.service('admin::api-token').revoke(id));
    });
  ...
  test('Creates a custom api token', async () => {
    balerion.contentAPI.permissions.providers.action.keys = jest.fn(() => [
  ...
  test('Fails to create a custom api token with unknown permissions', async () => {
    balerion.contentAPI.permissions.providers.action.keys = jest.fn(() => ['action-A', 'action-B']);
