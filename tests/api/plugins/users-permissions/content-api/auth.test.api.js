...
describe('Auth API', () => {
  beforeAll(async () => {
    ...
    const { jwt, user } = await createAuthenticatedUser({ balerion, userInfo: internals.user });

    data.user = user;

    rq = createRequest({ balerion }).setURLPrefix('/api/auth').setToken(jwt);
  });

  afterAll(async () => {
    await balerion.db.query('plugin::users-permissions.user').deleteMany();
    await balerion.destroy();
  });

  describe('Change Password', () => {
    test('Fails on unauthenticated request', async () => {
      const nonAuthRequest = createRequest({ balerion });
    ...
    test('Can login with new password after success', async () => {
      const rq = createRequest({ balerion }).setURLPrefix('/api/auth');
  ...
  test('Can login with password that previously validated but is now too long', async () => {
    ...
    const userInfo = {
      ...
      email: 'longpassworduser@balerion.io',
      password: longPassword,
      confirmed: true,
      provider: 'local',
    };

    const { user } = await createAuthenticatedUser({ balerion, userInfo });
